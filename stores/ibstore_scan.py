...
class IBStore(with_metaclass(MetaSingleton, object)):
    ...
    def getdata(cls, *args, **kwargs):
        ...
        print("I am in in ibstore, getdata")
    ...
    def __init__(self):
        ...
        #*****************************ERIK ADDED CODE***************************************************************
        self.reqcount = 0 #throttle limits
    ...
    def start(self, data=None, broker=None):
        ...
        print("I am in ibstore, start")
        # Datas require some processing to kickstart data reception
        if data is not None:
            print("I am in ibstore, start, if data is not none")
            self._env = data._env
            ...
            print("I am in ibstore, start, if broker is not none")
            self.broker = broker
    ...
    #@sleep_and_retry
    #@limits(calls=15, period=5) #period in seconds (10 minutes = 600 seconds)
    def startdatas(self):
        # kickstrat datas, not returning until all of them have been done
        ts = list()
     
        for data in self.datas:
            print("I am in ibstore, startdatas")
           # self.throt_delay(self)
    ...
    def reuseQueue(self, tickerId):
        ...
        with self._lock_q:
            ...
            isind = self.iscash.pop(tickerId, None)  #***********************ERIK ADDED*********************
            ...
            self.isind[tickerId] = isind   #***********************************ERIK ADDED****************************
    ...
    def getTickerQueue(self, start=False):
        ...
        with self._lock_q:
            ...
            self.isind[tickerId] = False #***********************ERIK ADDED*****************************
    ...
    def cancelQueue(self, q, sendnone=False):
        ...
        self.isind.pop(tickerId, None) #***************************ERIK ADDED***********************************
    ...
    def getContractDetails(self, contract, maxcount=None):
        ...
        if not cds or (maxcount and len(cds) > maxcount):
            err = f'{contract.m_symbol} Ambiguous contract: none/multiple answers received'
    ...
    def reqHistoricalDataEx(self, contract, enddate, begindate,
        ...
        #print("HITTING REQ HISTORICAL DATA EX ", contract.m_symbol)
        '''
        ...
        if contract.m_secType in ['IND']:  #******************ERIK ADDED 'IND'********************
            self.isind[tickerId] = 4  # msg.field code
            if not what:
                what = 'TRADES'  # default for cash unless otherwise specified********************************
    ...
    def reqHistoricalData(self, contract, enddate, duration, barsize,
        ...
        print("HITTING REQ HISTORICAL DATA ", contract.m_symbol)
        # get a ticker/queue for identification/data delivery
        tickerId, q = self.getTickerQueue()
        ...
        if contract.m_secType in ['IND']: #******************ERIK ADDED 'IND' BLOCK********************
            self.isind[tickerId] = True
            if not what:
                what = 'TRADES'  # TRADES doesn't work
        else:
            what = what or 'TRADES' #*******************************************************************
    ...
    def reqMktData(self, contract):
        ...
        if contract.m_secType in ['IND']:  #*******************************ERIK ADDED so TICK-NYSE CAN LOAD*********************
            self.isind[tickerId] = True
            ticks = ''  # cash markets do not get RTVOLUME********************************************************************
    ...
    def tickPrice(self, msg):
        ...
        ind_fieldcode = self.isind[tickerId] #******************************ERIK ADDED***************************
        
       #**********************ERIK ADDED BLOCK************************************ 
        if ind_fieldcode:
            #print("In tickPrice function, ind_fieldcode", msg.field, fieldcode)#*****************ERIK ADDED*******************************
            if msg.field == 4:  #***********************ERIK ADDED "or msg.field==4"***********************

                try:
                    rtvol = RTVolume(price=msg.price, tmoffset=self.tmoffset)
                    #print('In tickPrice function, after rtvol call:', msg.field, rtvol.price,rtvol.datetime) #*******ERIK ADDED***************
                except ValueError:  # price not in message ...
                    pass
                else:
       ...
       #*************************ERIK ADDED BLOCK**********************************
        ...
        if fieldcode:
            #print("In tickPrice function, fieldcode", msg.field, fieldcode)#*****************ERIK ADDED*******************************
            if msg.field == 1:  #***********************ERIK ADDED "or msg.field==4"***********************
                #********************************************ERIK REMOVED CODE SO TICK-NYSE DATA CAN LOAD**************************
    ...
    def historicalData(self, msg):
        ...
        q.put(msg)
        #print("Loading Historical Data (from Historical Data Wrapper capturing info back from IB")  #*********************************erik added *****************************************
