...
#Make dataframe with date range and ticker values
...
for i in tickers:
	setdf = newdf.assign(ticker=i)
	concat_list.append(setdf)
	
concat_df = pd.concat(concat_list)
concat_df['key'] = concat_df['ticker'].map(str) + concat_df.index.map(str)
#print(concat_df)
...
# Returns a list of all available earnings in tickers list
for i in tickers:
...
alldf.drop_duplicates(subset = ["datetime","ticker2"], keep = 'last', inplace = True) 
alldf.set_index('datetime', inplace=True)
alldf['key'] = alldf['ticker2'].map(str) + alldf.index.map(str)
alldf.rename(columns = {'ticker2':'ticker'}, inplace = True) 
#alldf = alldf.drop(['ticker2'], axis = 1) 
pd.set_option('display.max_rows', None)

print(alldf)
"""
#Merge dataframes
merged_df = concat_df.merge(alldf,on='key',how='outer')

#Replace 'Null' values with N/A values for each column
merged_df.set_index(concat_df.index, inplace=True)
merged_df["eps_estimate"].fillna(0, inplace = True)
merged_df["eps_actual"].fillna(0, inplace = True) 
merged_df["eps_diff"].fillna(0, inplace = True)
merged_df["source"].fillna("Yahoo Earnings Cal", inplace = True)   
#print(merged_df)
"""
...
"""
alldf.to_sql('fundamentals', engine, index=True,if_exists='append')
