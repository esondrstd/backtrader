...
#python -m cProfile "C:\Program Files\Python38\Lib\site-packages\backtrader\myscripts\Trading Package\Working Trading Modelv158.py"  #Type this in at command prompt to profile code, make sure to cd\ first
...
class UserInputs():
	...
	def datalist(data_req):
		...
		#datalist = ['TICK-NYSE','SPY','INTC','AAPL','AMZN','AON','APD', 'APH', 'ASML', 'ATVI', 'AVGO', 'AWK', 'AXP', 'AZO']
		datalist = ['SPY','AMT', 'AMZN', 'ANTM', 'AON', 'APD', 'APH', 'ASML', 'ATVI', 'AVGO', 'AWK', 'AXP', 'AZO', 'BA', 'BABA', 'BAC', 'BAX', 'BDX', 'BIDU', 'BIIB', 'BK', 'BKNG', 'BLK', 'BMRN', 'BMY', 'BSX', 'C', 'CAT', 'CB', 'CCI', 'CDNS', 'CERN', 'CHKP', 'CHTR', 'CI', 'CL', 'CLX', 'CMCSA', 'CME', 'CNC', 'COF', 'COP', 'COST', 'CRM', 'CSCO', 'CSX', 'CTAS', 'CTSH', 'CTXS', 'CVS', 'CVX', 'D', 'DD', 'DE', 'DG', 'DHR', 'DIS', 'DLR', 'DLTR','DUK', 'EA', 'EBAY', 'ECL', 'ED', 'EL', 'EMB', 'EMR', 'EQIX', 'EQR', 'ES', 'ETN', 'EW', 'EWH', 'EWW', 'EXC']
	...
	def model_params():
		params = dict(
			...
			start_date = date(2020,1,1), #Dates for backtesting
...
class Strategy(bt.Strategy):
	...
	params = dict(
			...
			ema1 = 5,  #8
			ema2 = 10, #20
			z_entry = 1.0,  #significance level required to enter pairs trade
	...
	def __init__(self):
		...
		self.trade_start = int(9)*60 + int(0)  #Trade Entry Start Hour and Minute
		self.trade_end = int(14)*60 + int(0)	#Trade Entry End Hour and Minute
		...
		self.datacount = self.modelp.get('t0_on') + self.modelp.get('t1_on') + self.modelp.get('t2_on')
...
#*********************************************INITITIALIZE INDICATORS*********************************************************				
				...
				self.inds[d._name]['atr_stop'] = btind.atr_stop(d,self.inds[d._name]['atr'],live=False,atrdist = self.p.atrdist,dollars_risked = self.p.total_dollars_risked,dollars_per_trade = self.p.dollars_risked_per_trade,plot=False)
		...
		#FOR PAIRS TRADING
			if d._name==d._name[:-1]+'0'and not d._name == 'TICK-NYSE0':
...
#**************************************************************************************************************************************
	...
	def notify_order(self, order):
		if order.status == order.Completed:
			...
			if order.isbuy() and self.pos < 0:
				print(f"{order.data._name} EXIT SHORT POSITION, Date: {self.dt} Price: {order.executed.price}, Cost: {order.executed.value}, Size {order.executed.size}, Type {order.getordername()}")
			...
			if order.issell() and self.pos > 0:
				print(f"{order.data._name} EXIT LONG POSITION, Date: {self.dt}  Price: {order.executed.price}, Cost: {order.executed.value}, Size {order.executed.size}, Type {order.getordername()}")
	...
	def notify_trade(self, trade):
		if trade.isclosed:
			print(f"{trade.data._name} POSITION CLOSED {self.dt} Price: {trade.price}, Net Profit: {trade.pnl}")
	...
	def next(self):
		...
		self.dt = self.datetime.datetime(ago=0)
		...
		if self.first_run_complete:
			self.pairs_entry_exit(self.p.z_entry)
		
		if self.hour==14 and self.minute==55 and self.pos:  #can't exit at 14:55 because cerebro submits at order at open of next bar
			self.eod_exit(d)
		#EXIT MULTIDAY POSITIONS
		#self.exit_multiday()
		
		"""
		#-------------------STOCK SELECTION BASED ON OPEN CRITERIA-----------------------------------------
		if self.hour==9 and self.minute==0:
			self.clear_dicts()	
		...
		if self.hour==9 and self.minute==5 and self.data._name==self.data0._name and self.perc_chg_dict is not None:
			self.rank_perc(self.data)
			#print(self.ranked_keys)
		
		#Iterate through ranked stocks only
		if self.ranked_keys:
			for n in self.ranked_keys:
				...
				if self.pos:
					if self.tick_close[0]>=1000:
						self.exit_trade(d,'long')
					elif self.tick_close[0]<=-1000:
						self.exit_trade(d,'short')
				
				#---------------------------------ENTRY LOGIC FOR LONG AND SHORT-----------------------------------------
				if d._name==d._name[:-1]+'0' and self.pos==0 and d._name[:-1] != 'TICK-NYSE' and self.entry_rules(d):  #Check that entry rules are met: 		
				
				#if d._name==d._name[:-1]+'0' and self.pos==0 and self.entry_rules(d):  #Check that entry rules are met: 
					#sig = self.sup_res(d)
					#sig = self.pull_back(d)
					#sig = self.tick_trade(d)
					#sig = self.hammer(d)
					#sig = self.engulfing(self.data)
					#sig = self.three_line(d)	
					#sig = self.signal_morn_break(d)
					sig = self.mean_revert(d)
					#sig=self.vwap_test(d)
					if sig=='buy':
						self.buyorder(d)
	...
	def create_pairs(self,d,signif):
		...
		"""	
		if len(d) > self.p.pairs_lookback * (self.modelp.get('timeframe0')/self.modelp.get('timeframe0')):  #ensure enough data has loaded for lookback period
	...
	def tick_trade(self,d):
		if self.tick_close[-1] < 0 and self.tick_close[0] > 0 and d._name in self.rtop_dict.keys():
			return 'buy'
		elif self.tick_close[-1] > 0 and self.tick_close[0] < 0 and d._name in self.rbot_dict.keys():
			return 'sell'
		else:
			return False	
	...
	def rank_perc(self,d):
		...
		if self.modelp.get('t0_on'):
			for k in self.merged_dict.keys():
				self.ranked_keys.append(k)
			self.ranked_keys.append('TICK-NYSE0')
	
		if self.modelp.get('t1_on'):
			for k in self.merged_dict.keys():
				self.ranked_keys.append(k[:-1]+'1')
			self.ranked_keys.append('TICK-NYSE1')
		
		if self.modelp.get('t2_on'):
			for k in self.merged_dict.keys():
				self.ranked_keys.append(k[:-1]+'2')
			self.ranked_keys.append('TICK-NYSE2')
