...
class Strategy(bt.Strategy):
	...
	def notify_order(self, order):
		...
		if order.status == order.Completed:
			...
			order.ref,
			round(order.size,2),
			'NA' if not order.price else round(order.price,2),
			))
	...
	def notify_trade(self, trade):
		...
		if trade.isclosed:
			print('-'*80)
			print('{}, TRADE CLOSED - Price: {}, Net Profit {}'.format(
			date,
			round(trade.price,2),
	...
	def next(self):
		...
		if self.hourmin=='08:35' and self.gap_dict is not None:
			...
			self.sorted_dict = {d: v for d,v in sorted(self.rtop_dict.items(),key=lambda x:x[1],reverse=True)}  #Same as above, except converted to dictionary so we can extract ticker 'keys'.  d._name is the key, and v is the value .  Reverse = true sorts greatest first
			self.sorted_tickers = self.sorted_dict.keys() #Get just the tickers ('keys')
			print(self.sorted_tickers)
			self.sortflag = 1
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if self.sorted_tickers is not None and self.name_t0 in self.sorted_tickers:
...
#********************************EXIT LOGIC/ORDERS*********************************************
				...
				if (d._name == self.name_t0 
					...
					self.exit_short_name = '{} - EXIT SHORT!!'.format(self.name_t0)        
				...
				#EXIT LOGIC FOR LONGS
					...
					self.exit_long_name = '{} - EXIT LONG!!'.format(self.name_t0)
				...
				if (d._name==self.name_t0
					...
					if(	#Timeframe 0 signals
						...
						if not self.modelp.get('live_status'):
							...
							#Create Long Order
							long_name = '{} - BUY BUY BUY'.format(self.name_t0)
							...
							long_stop_name = '{} - STOPPED OUT OF LONG TRADE'.format(self.name_t0)
					...
					#DEFINE SHORT ENTRY LOGIC
							...
							if not self.modelp.get('live_status'):
								...
								short_name = '{} - SELL SELL SELL'.format(self.name_t0)
								...
								short_stop_name = '{} - STOPPED OUT OF SHORT TRADE'.format(self.name_t0)
