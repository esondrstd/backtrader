...
class UserInputs():
	...
	def datalist(data_req):
		...
		#EXCEPTIONS - 'XEL',WDAY,UAL,SHY,ROKU,PEP,EMB,DOW,DD
...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.minimum_data = int(self.ticker_count)  #since iterating over 5 min periods, and longest timeframe is 1 hour, there are 12 5 min periods in an hour
	
		#Determine # of base timeframe periods within trading day
		self.intraday_periods = 1
	...
	def next(self):
		...
		#Print when year changes
		if self.year:
			self.p_year = self.year
		
		self.year = int(datetime.strftime(self.data.num2date(),'%Y'))
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			#self.slope_sma_width_t0 = round(self.inds.get(self.name_t0).get('slope_sma_width')[0],3)
			
			#self.slope_sma1_t0 = self.inds.get(self.name_t0).get('slope_sma1')[0]
			#self.slope_of_slope_sma1_t0 = self.inds.get(self.name_t0).get('slope_of_slope_sma1')[0]
			
			
			#Calculate RSI
			#self.rsi_t1 = round(self.inds.get(self.name_t0).get('rsi')[0],2)
			...
			#self.adx_t0 = round(self.inds.get(self.name_t0).get('adx')[0],3)
			
			#Calculate highest and lowest indicators
			#self.highest_t0 = round(self.inds.get(self.name_t0).get('highest')[0],3) 
			#self.lowest_t0 = round(self.inds.get(self.name_t0).get('lowest')[0],3) 
			...
			if (
				self.cash_avail > self.p.total_dollars_risked
			...
			#********************************EXIT LOGIC/ORDERS*********************************************
				...
				elif (self.dt == self.modelp.get('end_date') - timedelta(days=2)
					):
...
#*******************************************************************************************************************************
	if not modelp.get('live_status'):
		...
		for i,j in enumerate(datalist):
			...
			dailt_Timeframe = cerebro.resampledata(data,
										name="{}0".format(j),
...
#*****************************************************************************************************************************
		...
		for i,j in enumerate(ibdatalist):
			...
			daily_Timeframe = cerebro.resampledata(data,
										name="{}0".format(j),
										timeframe=bt.TimeFrame.Days,
										compression = 1,
										)
