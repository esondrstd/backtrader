...
#*********************************************INITITIALIZE INDICATORS*********************************************************				
				...
				self.inds[d._name]['prior_day'] = btind.priorday(d,period=79)
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hour==8 and self.minute==30:
			...
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters					
				if d._name == d._name[:-1]+'0' and d._name!='TICK-NYSE0':	
					...
					#self.prior_day(d)  #set dictionary for prior day high, low, and close
		...
		for n in self.merged_dict.keys():
			...
			if self.hour<11 and self.minute<=30 and self.pos:
				self.percK_t0 = self.inds.get(d._name).get('stochastic').lines.percK[0]
				if self.percK_t0 > 85 or self.tick_close[0]>800:
					self.exit_trade(d,'long')
				elif self.percK_t0 < 85 or self.tick_close[0]<=-800:
					self.exit_trade(d,'short')
			
			#---------------------------------ENTRY LOGIC FOR LONG AND SHORT-----------------------------------------
			if self.hour==8 and self.minute==30 and d._name==d._name[:-1]+'0' and d._name[:-1] != 'TICK-NYSE' and self.entry_rules(d):  #Check that entry rules are met
	...
	def clear_dicts(self):
		...
		self.high_dict.clear()
		self.low_dict.clear()
		self.close_dict.clear()
	...
	def prior_day(self,d):
		if self.hour == 8 and self.minute == 30:
			self.prior_high = max(d.high.get(size=79))
			self.high_dict[d._name].append(self.prior_high)
			self.prior_low = min(d.low.get(size=79))
			self.low_dict[d._name].append(self.prior_low)
			self.prior_close = d.close[-1]
			self.close_dict[d._name].append(self.prior_close)
	...
	def gap_revert(self,d):
		...
		#prior_high = self.high_dict.get(d._name)[0]
		#prior_low = self.low_dict.get(d._name)[0]
		#prior_close = self.close_dict.get(d._name)[0]
		#print(d._name,d.close[0],prior_high,prior_low,prior_close)
		
		#if d._name in self.rbot_dict.keys() and d.close[0]<prior_low:
		if d._name in self.rbot_dict.keys() and self.gap_dict.get(d._name) < -2 and self.slope_t0 > 0:
			return 'buy'
		elif d._name in self.rtop_dict.keys() and self.gap_dict.get(d._name) > 2 and self.slope_t0 < 0:
		#if d._name in self.rtop_dict.keys() and d.close[0]>prior_high: 
			return 'sell'
