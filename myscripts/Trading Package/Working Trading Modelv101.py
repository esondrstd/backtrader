...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.short_stop_list = defaultdict(list)
		self.long_stop_list = defaultdict(list)
		...
		for i, d in enumerate(self.datas):
			...
			if d._name==self.name_t0:
				...
				self.short_stop_list[d._name].append(0)
				self.long_stop_list[d._name].append(0)
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if d._name==self.name_t0 and d._name in self.combosorted_dict.keys() and self.combosorted_dict is not None:
				...
				#print(d._name,self.dt,self.hourmin, self.inorder_dict.get(d._name)[-1],self.pos)
				if (self.inorder_dict.get(d._name)[-1] == True and self.inorder_dict.get(d._name)[-2]!= False and self.pos==0):
					self.inorder_dict[d._name].append(False)
					self.size_dict[d._name].append(0)
				...
				#EXIT LOGIC FOR LONGS
				elif (self.name_t0 == self.name_t0 
					#and not (d._name[:-1]=='VIX' or d._name[:-1]=='TICK-NYSE' or d._name[:-1]=='TRIN-NYSE' or d._name[:-1]=='SPY')	
					and self.size_dict.get(d._name)[-1] > 0
					and self.inorder_dict.get(d._name)[-1] == True
					...
					#CANCEL STOPS BEFORE EXITING
					print(f"{d._name} Prepare to exit trade - close STOP positions")
					print(dir(self.long_stop_list.get(d._name)[-1]))
					print(self.long_stop_list.get(d._name)[-1].Status)
					if self.long_stop_list.get(d._name)[-1].Status != "Completed":  #there is Status=="Completed" and status==4 means completed
						self.cancel(self.long_stop_list[-1])
					#print(self.long_stop_list[0])
				...
				if (not self.name_t0[:-1]=='TICK-NYSE'
					...
					if(	#Timeframe 0 signals
						...
						if not self.modelp.get('live_status'):
							...
							self.long_stop_list[d._name].append(self.long_stop_ord)
							self.pos=self.size
					...
					#DEFINE SHORT ENTRY LOGIC
							...
							if not self.modelp.get('live_status'):
								...
								self.short_stop_list[d._name].append(self.short_stop_ord)
								self.pos=self.size
