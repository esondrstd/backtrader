...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.sorted_tickers = defaultdict(list)
		...
		for i, d in enumerate(self.datas):
			...
			if d._name==self.name_t0:
				...
				self.sorted_tickers[d._name].append(0)
				self.ranked_dict[d._name].append(0)
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			"""
			#self.ranked_dict.clear()  #deletes dictionary at start of day (for memory optimization/sort speed)
			...
			if self.hourmin=='08:30' and d._name==self.name_t0:
				...
				self.ranked_dict[d._name].append(self.gap) #add rank to dictionary
			...
			print(self.sorted_tickers.keys())
			"""
			...
			self.target_short = self.target_short_dict.get(self.name_t0)[-1]
			self.target_long = self.target_long_dict.get(self.name_t0)[-1]
			...
			print(d._name,self.dt,self.hourmin, self.inorder_dict.get(self.name_t0)[-1],self.pos)
			if (self.inorder_dict.get(self.name_t0)[-1] == True and self.inorder_dict.get(self.name_t0)[-2]!= False and self.pos==0):
				self.inorder_dict[self.name_t0].append(False)
				self.size_dict[self.name_t0].append(0)
				print('{} {} Stopped Out!! {} - {} shares at {}'.format(self.dt, self.hourmin, self.name_t0, self.size_dict[self.name_t0][-2], self.stop_dict[self.name_t0][-1]))
			...
			if (not self.name_t0[:-1]=='TICK-NYSE'
				...
				and self.size_dict.get(self.name_t0)[-1] == 0
				and self.inorder_dict.get(self.name_t0)[-1] == False
				...
				if(	#Timeframe 0 signals
					...
					if not self.modelp.get('live_status'):
						...
						#Create Long Order
						long_name = '{} - Enter Long Trade'.format(self.name_t0)
						...
						long_stop_name = '{} - Fixed StopLoss for Long Entry'.format(self.name_t0)
						...
						self.size_dict[self.name_t0].append(self.size)
						#Track if currently in an order or not
						self.inorder_dict[self.name_t0].append(True)
						...
						self.target_long_dict[self.name_t0].append(self.target_long_price)
						self.stop_dict[self.name_t0].append(self.long_stop)
						self.pos=self.size
				...
				#DEFINE SHORT ENTRY LOGIC
						...
						if not self.modelp.get('live_status'):
							...
							short_name = '{} - Enter Short Trade'.format(self.name_t0)
							...
							short_stop_name = '{} - Fixed StopLoss for Short Entry'.format(self.name_t0)
							...
							self.target_short_dict[self.name_t0].append(self.target_short_price)
							self.stop_dict[self.name_t0].append(self.short_stop) #dictionary needed by cancel order to access all symbols instead of last symbol
							self.pos=self.size
							...
							#print('**************SHORT SIGNAL**************')
							#Create Short Entry Order
							short_name = '{} - Short Entry'.format(self.name_t0)
							self.short_ord = self.sell(data=self.name_t0,
												size=self.size,
												exectype=bt.Order.Market,
												transmit=False,
			...
			#********************************EXIT LOGIC/ORDERS*********************************************
				...
				if (self.name_t0 == self.name_t0 
					...
					and self.size_dict.get(self.name_t0)[-1] < 0
					and self.inorder_dict.get(self.name_t0)[-1] == True
					and self.prenext_done #Start trading after all prenext data loads 
					...
					self.exit_short_name = '{} - Exit Short Trade'.format(self.name_t0)        
					self.exit_short = self.close(self.name_t0,
												size= self.size_dict.get(self.name_t0)[-1],
												name=self.exit_short_name)
					
					print('{} {} EXIT SHORT {} - {} shares at {}'.format(self.dt, self.hourmin, self.name_t0,self.size_dict.get(self.name_t0)[-1],d.close[0]))
					self.inorder_dict[self.name_t0].append(False)
					self.size_dict[self.name_t0].append(0)
				...
				#EXIT LOGIC FOR LONGS
					...
					print('{} {} EXIT LONG {} - {} shares at {}'.format(self.dt, self.hourmin, self.name_t0,self.size_dict.get(self.name_t0)[-1],d.close[0]))
					#LONG EXIT ORDER - closes existing position and cancels outstanding stop-loss order
					self.exit_long_name = '{} - Exit Long Trade'.format(self.name_t0)
