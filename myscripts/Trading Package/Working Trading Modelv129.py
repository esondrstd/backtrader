...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if self.exit_pair_dict.get(f'exit_sig {name}')[0]
	...
	def pairs_entry(self,d):
		...
		for i in self.pair_spread_dict:
			name = i[7:]
			if self.long_pair_dict.get(f'long_sig {name}')[0] and self.pos==0 and self.entry_rules(d):
				self.buyorder(d)
				
			if self.short_pair_dict.get(f'short_sig {name}')[0] and self.pos==0 and self.entry_rules(d):
				self.buyorder(d)
	...
	def adfuller_test(self,ols_results,t1_name,t2_name,signif):
		...
		print("{self.dt} {self.hourmin} NUMBER OF PASSING ADF PAIRS",len(self.adfpval))	
