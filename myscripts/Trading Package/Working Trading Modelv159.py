...
#IMPORT MODULES
#python -m cProfile "C:\Program Files\Python38\Lib\site-packages\backtrader\myscripts\Trading Package\Working Trading Modelv159.py"  #Type this in at command prompt to profile code, make sure to cd\ first
...
class UserInputs():
	...
	def datalist(data_req):
		...
		ibdatalist = ['MCD','SPY','AAPL']  #'AAPL-STK-SMART-USD'
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hour==9 and self.minute==0:
			self.clear_dicts()	
			#Get correlations to SPY
			self.myspy = self.spy_close.get(size=self.p.correl_per)
		...
		if self.merged_dict.keys():
			for n in self.merged_dict.keys():
				...
				if self.hour==14 and self.minute==50 and self.pos:  #can't exit at 14:55 because cerebro submits at order at open of next bar
					self.eod_exit(d)
				
				"""
				if self.pos:
					if self.tick_close[0]>=700:
						self.exit_trade(d,'long')
					elif self.tick_close[0]<=-700:
