...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2017,10,2), #Dates for backtesting
...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		for i, d in enumerate(self.datas):
			...
			"""
			self.inds[d._name]['mytick'] = btind.Tick(self.data6,
											plot=True)
			"""
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			"""
			mytick = self.inds.get(self.name_t0).get('mytick')[0]
			print(d._name,self.dt, self.hourmin, mytick)
			"""
			...
			self.engulfing_pattern_t0= round(self.inds.get(self.name_t0).get('engulfing')[0],3)
