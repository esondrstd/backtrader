...
#python -m cProfile "C:\Program Files\Python38\Lib\site-packages\backtrader\myscripts\Trading Package\Working Trading Modelv157.py"  #Type this in at command prompt to profile code, make sure to cd\ first
...
class UserInputs():
	...
	def datalist(data_req):
		...
		#datalist = ['TICK-NYSE','SPY','INTC','AAPL','AMZN','AON']
	...
	def model_params():
		params = dict(
			...
			#start_date = date2num(date('2019-01-01')),
			#end_date = date2num(date('2019-01-10')),
...
#**************************************************************************************************************************************
	...
	def nextstart(self):
		#Theere is a nextstart method which is called exactly once, to mark the switch from prenext to next. 
	...
	def next(self):
		...
		if self.hour==9 and self.minute==5 and self.perc_chg_dict is not None and self.data._name==self.data0._name:
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if self.hour==9 and self.minute==0 and d._name==d._name[:-1]+'0' and d._name!='TICK-NYSE0':
			...
			"""
			if self.pos and d._name == d._name[:-1]+'0':
				#if self.tick_close[0]<=-1000
				if self.target_long:
					self.exit_trade(d,'long')
				elif self.target_short:
					self.exit_trade(d,'short')
			"""
			#---------------------------------ENTRY LOGIC FOR LONG AND SHORT-----------------------------------------
			if d._name==d._name[:-1]+'0' and d._name in self.merged_dict.keys() and self.pos==0 and d._name!='TICK-NYSE0' and self.entry_rules(d):  #Check that entry rules are met: 		
	...
	def pairs_entry_exit(self,z_entry_threshold=2.0, z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			if self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and (exit_signal or t1_long_stop_sig or t2_long_stop_sig or t1_short_stop_sig or t2_short_stop_sig or pair_stop_signal or (self.hour==14 and self.minute==45)):
