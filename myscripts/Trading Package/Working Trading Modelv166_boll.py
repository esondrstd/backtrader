...
USE VECTOR MULTIPLICATION/ADDITION whenever you can - speeds up program tremendously
"""

#IMPORT MODULES
#python -m cProfile "C:\Program Files\Python38\Lib\site-packages\backtrader\myscripts\Trading Package\Working Trading Modelv163_gap revert.py"  #Type this in at command prompt to profile code, make sure to cd\ first
...
class IB_Scan():
	def __init__(self):
		...
		self.ticker_list_gain = []
		self.ticker_list_loss = []
		
	def ib_gainers(self):
		#subscribe to IB market scanner and return stock list
		
		TopPerGain = ScannerSubscription()
		TopPerGain.scanCode = 'HIGH_OPEN_GAP'
		TopPerGain.numberOfRows = 3
		TopPerGain.locationCode = 'STK.US.MAJOR'
		TopPerGain.instrument = 'STK'
		TopPerGain.m_abovePrice = 5
		TopPerGain.m_aboveVolume = 1000000
		 
		topPerGain = self.ib.reqScannerData(TopPerGain, [])
		#print( 'topPerGain =' , topPerGain)

		for stock in topPerGain[:10]: # loops through the first 10 stocks in the scanner
			rank = stock.rank
			...
			#print('GAINERS   Stock:', rank, secType, symbol, exchange) 
			self.ticker_list_gain.append(symbol)
		
		#self.ib.disconnect()
		print(self.ticker_list_gain)
		return self.ticker_list_gain
	
				
	def ib_losers(self):
		#subscribe to IB market scanner and return stock list
		TopPerLoss = ScannerSubscription()
		TopPerLoss.scanCode = 'LOW_OPEN_GAP'
		TopPerLoss.numberOfRows = 3
		TopPerLoss.locationCode = 'STK.US.MAJOR'
		TopPerLoss.instrument = 'STK'
		TopPerLoss.m_abovePrice = 5
		TopPerLoss.m_aboveVolume = 1000000
		 
		topPerLoss = self.ib.reqScannerData(TopPerLoss, [])
		#print( 'topPerLoss =' , topPerLoss)


		for stock in topPerLoss[:10]: # loops through the first 10 stocks in the scanner
			rank = stock.rank
			contract = stock.contractDetails.contract
			secType = stock.contractDetails.contract.secType
			conId = stock.contractDetails.contract.conId
			...
			#print('LOSERS   Stock:', rank, secType, symbol, exchange) 
			self.ticker_list_loss.append(symbol)
		
		self.ib.disconnect()
		print(self.ticker_list_loss)
		return self.ticker_list_loss
...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2020,2,1), #Dates for backtesting
...
class Strategy(bt.Strategy):
	"""Class to define indicator and money management parameters"""
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hour==8 and self.minute==30:
			...
			if not self.modelp.get('live_status'):	
				...
				print("YO YO YO", self.ticker_list)
		...
		if self.hourmin >= time(8,45) and self.hourmin <= self.trade_end:
			for n in self.ticker_list:
				...
				if self.hour==14 and self.minute==50:  #can't exit at 14:55 because cerebro submits at order at open of next bar
					self.eod_exit(d)
				...
				if self.hourmin >= self.trade_start:  #Need sufficient number of bars to pass so you don't get stopped too early from stochastic
					...
					self.boll_mid_t0 = self.inds.get(d._name[:-1]+'0').get('bollinger').lines.mid[0] 
					#self.slope_obv_t0 = self.inds.get(d._name[:-1]+'0').get('slope_obv')[0] #Get OBV
					
					if self.pos > 0:
						if self.percK_t0 > 90 or d.close[0] >= self.boll_mid_t0:
							self.exit_trade(d,'long')  #exit long trade
					
					if self.pos < 0:
						if self.percK_t0 < 10 or d.close[0] <= self.boll_mid_t0:
	...
	def gap_revert(self,d):
		...
		if d._name in self.rbot_dict.keys() and self.gap_dict.get(d._name) < -5:
			return 'buy'
	
		elif d._name in self.rtop_dict.keys() and self.gap_dict.get(d._name) > 5:
			return 'sell'
...
#*************************************************************************************************************************************
#Define Global Variables needed to run strategy
...
"""
def throt_delay():
	#Needed to avoid Interactive Broker Pacing Restrictions for too many tickers requested
	global reqcount, lastreqtime
	print(reqcount)
	throttime = 1 / reqlmt  # throttle delay in fraction of seconds
	reqcount += 1  # add one throt period
	reqcount -= int((t.time() - lastreqtime) / throttime)          # Decrement count by time passed:
...
def data_live(cerebro,session_start,session_end,modelp):
	...
	ibdatalist_gainers = IB.ib_gainers()
	ibdatalist_losers = IB.ib_losers()
	ibdatalist = ibdatalist_gainers + ibdatalist_losers
	#ibdatalist = ['JFKKU', 'KOLD', 'VTGN']
	#ibdatalist = ('EUR','GBP','AUD')
	print("THE LIST",ibdatalist)
	###*******MAY NEED TO INSERT SOME KIND OF TIMER TO MAKE THIS WORK
...
def open_IB():
	...
	while current_time < hourmin or (current_time > time(15,0) and current_time <= time(23,59,59)):
		...
		print(f'{current_time} - Waiting to open Interactive Brokers')
		t.sleep(60)
