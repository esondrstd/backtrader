...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','AAPL','INTC']
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hourmin=='08:35' and not self.df_returns.empty:
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if self.hourmin=='08:35' and d._name==d._name[:-1]+'1' and d._name!='TICK-NYSE1' and not self.df_returns.empty:
				#self.plotdf(d,self.df_closes)
				self.plotratio(d,self.df_closes)
				self.coint(d,self.df_closes)
				self.adfull(d,self.df_closes)
	...
	def create_df(self,d):
		...
		print("Dataframe of Closes",self.df_closes)
	...
	def coint(self,d,df):
		...
		if d._name!='SPY1':
			...
			cutoff = .005
			print(f'{d._name} Cointegration Results: {pvalue}') 
			if pvalue < cutoff:
				print(f'p-value = {pvalue}. The {d._name} and SPY1 pair is likely stationary via cointegration test.')
			else:
				print(f'p-value = {pvalue}. The {d._name} and SPY1 pair is likely NOT stationary via cointegration test.')
	
	
	
	def plotratio(self,d,df):
		if d._name!='SPY1':
			ratio = df['SPY1']/df[d._name]
			plot1 = ratio.plot.line(title=f'Price Ratio between {d._name} and SPY1')
			plt.axhline(ratio.mean(), color='red', linestyle='--')
			plot1.set_ylabel('Price Ratio')
			plt.show()
	
	
	def plotdf(self,d,df):
		#jres = coint_johansen(self.df, det_order=0, k_ar_diff=1)
		#print(jres)
		if d._name!='SPY1':
			plot1 = df[d._name].plot.line(legend=True,title=f'Historical Close Prices for {d._name} and SPY1')
			plot2 = df['SPY1'].plot.line(secondary_y=True,legend=True)
			plot1.set_ylabel(d._name)
			plot2.set_ylabel('SPY1')
			plt.show()

	def adfull(self,d,df):	
		"""ADFuller Unit root test on data series"""		
		...
		if d._name!='SPY1':
			...
			#for key, value in result[4].items():
				...
				#print(f'   {key}, {value}')
			if pvalue < cutoff:
				print(f'p-value = {pvalue}. The {d._name} series is likely stationary via ADFuller Test.')
			else:
				print(f'p-value = {pvalue}. The {d._name} series is likely NOT stationary via ADFuller Test.')
