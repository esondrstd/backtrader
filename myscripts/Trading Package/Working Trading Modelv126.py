...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hourmin=='08:35':
			...
			self.pair_regression(self.data)  #refine pairs list by performing ols regression than ADF test
			self.calc_spread_zscore(self.data)
			self.pairs_signal(self.data)
	...
	def pairs_signal(self,d,z_entry_threshold=1.0,z_exit_threshold=0):
		...
		self.first_run_complete = 1  #flag to indicate function was run
	
	
	def calc_spread_zscore(self,d):
	...
	"""
		if not self.first_run_complete:
			...
			for i in self.adfpval:
				...
				spread = self.justclose_dict.get(t1) - hratio*self.justclose_dict.get(t2)
				self.pair_spread_dict[f'spread {t1}/{t2}'].append(spread)
				
			for i in self.pair_spread_dict:
				name = i[7:]
				zscore = stats.zscore(self.pair_spread_dict.get(i).get(ago=0,size=self.p.pairs_lookback))
				self.pair_zscore_dict[f'zscore {name}'].append(zscore)
	"""
	...
	def pair_regression(self,d):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			Y = self.justclose_dict.get(stock1)
			beta = self.justclose_dict.get(stock2)
	...
	def create_pairs(self,d,signif):
		...
		df = pd.DataFrame.from_dict(self.justclose_dict)
	...
	def update_df(self,d):
		self.justclose_dict[d._name].append(d.close)
