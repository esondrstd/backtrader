...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if self.hourmin=='08:30' and d._name!='TICK-NYSE1' and d._name==d._name[:-1]+'1':		
				self.clear_pairs()
				self.justclose_dict[d._name] = self.inds.get(d._name[:-1]+'1').get('close').get(size=self.p.pairs_lookback)
			...
			if  d._name==d._name[:-1]+'1' and d._name!='TICK-NYSE1':
				self.justclose_dict[d._name].append(d.close[0]) #add new close value
				self.justclose_dict[d._name].pop(0)  #delete first close value in dictionary (keeps lookback period constant)
					
			#if d._name==self.data1._name:  #Just need date for 1 symbol	
				#self.justdate_dict[d._name].append(d.datetime[0])
				#self.justdate_dict[d._name].pop(0)
	...
	def create_pairs(self,d,signif):
		...
		for i, (ticker1, ticker2) in enumerate(all_pairs): 	
			...
			hedge_ratio = np.round(result.evec[:, 0],2)	#The first column of eigenvectors contains the best weights (shortest half life for mean reversion).  This determine shares of each instrument.
		...
		print("NUMBER OF COINTEGRATING PAIRS:", len(self.cointegrating_pairs),self.cointegrating_pairs)  #return number of cointegrating pairs		
		self.pair_regression()
		
		
	def pair_regression(self):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			hratio_t1 = pair.get('hratio_t1')		
			stock2 = pair.get('t2')	#X variable
			hratio_t2 = pair.get('hratio_t2')
			...
			self.adfuller_test(error,stock1,stock2,hratio_t1,hratio_t2,self.p.signif)
		
		print (f'Number of ADF PAIRS: {len(self.adfpval)} {self.adfpval}')

		
	def adfuller_test(self,ols,t1_name,t2_name,hratio_t1,hratio_t2,signif):
		...
		if p_value <= signif:
			self.adfpval.append(dict(t1=t1_name,t2=t2_name,hratio1 = hratio_t1,hratio2=hratio_t2))	
	...
	def calc_spread_zscore(self,z_entry_threshold=2.0):
		...
		for i in self.adfpval:			
			...
			hratio_t1 = i['hratio1']
			hratio_t2 = i['hratio2']
			t1_data = self.justclose_dict.get(t1)
			t2_data = self.justclose_dict.get(t2)
			pos_t1 = self.getpositionbyname(t1).size
			pos_t2 = self.getpositionbyname(t2).size
			
			#THESE ALL NEED TO BE IN SEPARATE DICTIONARIES TO BE REFERENCED OUTSIDE FUNCTION FOR SIZING
			self.net_pair_val = round((t1_data[-1]*hratio_t1) + (t2_data[-1]*hratio_t2),2)  #think market price of "spread", just like price per share for any stock.  Hedge ratio sign indicates whether you are long or short that instrument (i.e. - hedge ratio means short, positive long)
			self.num_units = self.p.total_dollars_risked/self.net_pair_val  #this represents how many "shares" of the pair spread you can long or short
			self.size_t1 = int(self.num_units*hratio_t1)
			self.size_t2 = int(self.num_units*hratio_t2)
			...
			spread = np.round(np.subtract(arrt1,arrt2),2)
			...
			self.long_pair_dict[f'long_sig {t1}/{t2}'].append(zscore.any() <= -z_entry_threshold*1.0)
			self.short_pair_dict[f'short_sig {t1}/{t2}'].append(zscore.any() >= z_entry_threshold*1.0)
			spread_stop = abs(zscore[-1]) >= z_entry_threshold + 1
			self.inorder_dict[name]['net_val'].append(self.net_pair_val)
			self.inorder_dict[name]['stop'].append(spread_stop)

			if self.hourmin=='08:35' and self.long_pair_dict.get(f'long_sig {name}'):
				if hratio_t1 > 0:
					#and not pos_t1 and not pos_t2 and self.cash_avail > self.p.total_dollars_risked:
					...
					transmit=True)
					self.inorder_dict[name][f'{t1} inorder'].append(True)
					self.inorder_dict[name][f'{t1} size'].append(self.size_t1)
					self.inorder_dict[name][f'{t1} direction'].append('long')
					#self.inorder_dict[name].append(dict(inorder=True,direction_t1='long',size_t1 = self.size_t1, net_val=self.net_pair_val,stop=spread_stop))
					print(f"{self.dt} {self.hourmin} For Pair {name} - Enter LONG Trade for leg {t1}")
				...
				if hratio_t2 > 0:
					...
					exectype=bt.Order.Market,
					transmit=True)
					self.inorder_dict[name][f'{t2} inorder'].append(True)
					self.inorder_dict[name][f'{t2} size'].append(self.size_t2)
					self.inorder_dict[name][f'{t2} direction'].append('long')
					#self.inorder_dict[name].append(dict(inorder=True,direction_t2='long',size_t2 = self.size_t2,net_val=self.net_pair_val,stop=spread_stop))
					print(f"{self.dt} {self.hourmin} For Pair {name} - Enter LONG Trade for leg {t2}")
				
			if self.hourmin=='08:35' and self.short_pair_dict.get(f'short_sig {name}'):
				if hratio_t1 < 0:
					...
					self.inorder_dict[name][f'{t1} inorder'].append(True)
					self.inorder_dict[name][f'{t1} size'].append(self.size_t1)
					self.inorder_dict[name][f'{t1} direction'].append('short')
					#self.inorder_dict[name].append(dict(inorder=True,direction_t1='short',size_t1 = self.size_t1,net_val=self.net_pair_val,stop=spread_stop))
					print(f"{self.dt} {self.hourmin} For Pair {name} - Enter SHORT Trade for leg {t1}")
				...
				if hratio_t2 < 0:
					...
					self.inorder_dict[name][f'{t2} inorder'].append(True)
					self.inorder_dict[name][f'{t2} size'].append(self.size_t2)
					self.inorder_dict[name][f'{t2} direction'].append('short')
					#self.inorder_dict[name].append(dict(inorder=True,direction_t2='short',size_t2 = self.size_t2,net_val=self.net_pair_val,stop=spread_stop))
					print(f"{self.dt} {self.hourmin} For Pair {name} - Enter Short Trade for leg {t2}")
	...
	def pairs_exit(self,z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			t1_data = self.justclose_dict.get(t1)
			t2_data = self.justclose_dict.get(t2)
			t1_size = self.inorder_dict[name].get(f'{t1} size')[0]
			t2_size = self.inorder_dict[name].get(f'{t2} size')[0]
			inorder_t1 = self.inorder_dict[name].get(f'{t1} inorder')[0]
			inorder_t2 = self.inorder_dict[name].get(f'{t2} inorder')[0]
			t1_direction = self.inorder_dict[name].get(f'{t1} direction')[0]
			t2_direction = self.inorder_dict[name].get(f'{t2} direction')[0]
			net_val = self.inorder_dict[name].get('net_val')[0]
			stop_price = self.inorder_dict[name].get('stop')[0]

			#print(self.dt,self.hourmin,name,t1,hratio_t1,t2,hratio_t2,t1_size,t2_size,inorder_t1,inorder_t2,t1_direction,t2_direction,net_val,stop_price)
			
			spread_now = round(hratio_t1 * t1_data[0] + hratio_t2 * t2_data[0],2) #create array of hedge ratio * close price
			self.pair_spread_dict[f'spread {t1}/{t2}'].append(spread_now)
			self.pair_spread_dict[f'spread {t1}/{t2}'].pop(0)
			zscore_now = stats.zscore(self.pair_spread_dict.get(f'spread {t1}/{t2}')[-self.p.pairs_lookback:])
			self.pair_zscore_dict[f'zscore {t1}/{t2}'].append(round(zscore_now[-1],4))  #appends, but appends outside of np array
			self.pair_zscore_dict[f'zscore {t1}/{t2}'].pop(0)
			exit_signal = abs(zscore_now[-1]) <= z_exit_threshold*1.0
			self.exit_pair_dict[f'exit_sig {t1}/{t2}'].append(exit_signal)
		print(self.hourmin)
		...
		if self.pair_spread_dict:
			...
			for i, pair in enumerate(self.adfpval):
				if i <= (rows*cols)-1:
					t1_name = pair.get('t1')
					t2_name = pair.get('t2')
					spread_data= self.pair_spread_dict.get(f'spread {t1_name}/{t2_name}')[0]
		...
		if self.pair_spread_dict:
			...
			for i, pair in enumerate(self.adfpval):
				if i <= (rows*cols)-1:
					...
					spread_data= self.pair_zscore_dict.get(f'zscore {t1_name}/{t2_name}')[0]
					ax = fig.add_subplot(rows, cols, i+1)
					ax.plot(spread_data,'g-')
