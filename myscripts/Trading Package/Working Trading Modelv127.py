...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hourmin=='08:35' and self.justclose_dict:
			...
			self.plot_pair() #Plot final pairs
			
		if self.justclose_dict:
			self.update_pairs(self.data)
	...
	def pairs_signal(self,d,z_entry_threshold=1.0,z_exit_threshold=0):
		...
		if self.long_pair_dict.get(f'long_sig {name}')[0]:
			return True
		elif self.short_pair_dict.get(f'short_sig {name}')[0]:
			return False
	...
	def calc_spread_zscore(self,d):
		...
		for i in self.adfpval:
			t1 = i['t1']
			t2 = i['t2']
			self.hratio_close_dict[t2] = [self.hratio*x for x in self.justclose_dict.get(t2)]  #mutiply stock 2 array by hratio
	...
	def create_pairs(self,d,signif):
		...
		"""
		self.coint_df.iloc[0:0]	#delete dataframe
	...
	def plot_pair(self):
		...
		for i, pair in enumerate(self.adfpval):
			...
			plot1 = ticker1.plot.line(legend=True,title=f'Historical Close Prices for {t1} and {t2}')
		...
		for i in self.adfpval:
			...
			ax.plot1 = ticker1.plot.line(legend=True,title=f'Historical Close Prices for {t1} and {t2}')
			ax.plot2 = ticker2.plot.line(secondary_y=True,legend=True)
			ax.plot1.set_ylabel(t1)
			ax.plot2.set_ylabel(t2)
