...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','XLK','A', 'AAL', 'AAPL', 'ABBV', 'ABT', 'ACN', 'ADBE', 'ADI', 'ADP']
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if self.entry_rules(d,d._name):  #Check that entry rules are met
				...
				if self.signal_morn_break(d,d._name,'long'):
					print("Outside Function Size",self.trade_size(d._name))
...
#RUN STRATEGY
	...
	self.analysis(cerebro,results[0]) #get all the result analysis
	#self.myplot(cerebro,results[0])	#plot results

#************************************************************************************************************************************			
def myplot(self,cerebro,results):
...
def analysis(self,cerebro,results):
