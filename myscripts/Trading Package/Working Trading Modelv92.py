...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		if not self.modelp.get('live_status'):
			...
			print('Ticker Count = ',self.ticker_count)
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			#print("PRE SORT",d._name, self.dt, self.hourmin, d.close[0],d.high[0],d.low[0],self.inorder_dict[d._name])
			...
			"""
			#print(self.dt,self.hourmin,self.sorted_dict.keys())
			self.destring_list.clear()
			for item in self.sorted_tickers.keys():
				destring = re.sub(r" ?\([^)]+\)", '', item)
				print(destring)
				self.destring_list.append(destring)
			print(self.destring_list)
			"""
			
			#print(d._name in self.sorted_tickers.keys())
			...
			#print("POST SORT",d._name, self.dt, self.hourmin, d.close[0],d.high[0],d.low[0],self.inorder_dict[d._name])
