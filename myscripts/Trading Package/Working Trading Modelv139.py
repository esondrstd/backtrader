...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if  d._name==d._name[:-1]+'1' and d._name!='TICK-NYSE1' and self.first_run_complete:
				self.justclose_dict[d._name] = self.inds.get(d._name[:-1]+'1').get('close').get(size=self.p.pairs_lookback)
	...
	def adfuller_test(self,ols,t1_name,t2_name,hratio_t1,hratio_t2,signif):
		...
		if p_value <= signif:
			...
			#print(f" ADF Test => P-Value = {p_value}. Rejecting Null Hypothesis that Data has unit root (non-stationary).")
	...
	def pairs_exit(self,z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			self.pair_zscore_dict[f'zscore {t1}/{t2}'].append(round(zscore_now,4))
			...
			if self.exit_pair_dict.get(f'exit_sig {t1}/{t2}')[-1] or self.hourmin=='14:50':
				self.close(t1)
				self.close(t2)
				print(f'EXIT pair {t1}/{t2}')
