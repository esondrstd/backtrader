...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','XLK']
	...
	def __init__(self):
		...
		for i, d in enumerate(self.datas):
			...
			if d._name==self.name_t0:
				...
				self.inorder_dict[d._name].append(0)
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if d._name==self.name_t0 and d._name in self.combosorted_dict.keys() and self.combosorted_dict is not None:
				...
				if (d._name == self.name_t0 
					...
					print(self.inorder_dict.get(d._name)[-1])  
					
					"""
					print(f"{d._name} Prepare to exit trade - close STOP positions")
					print(self.short_stop_list.get(d._name)[-1].exectype)
					
					print(self.short_stop_list.get(d._name)[-1])
					if self.short_stop_list.get(d._name)[-1].status != 4:  #4 means completed
						self.cancel(self.short_stop_list[-1])
					#print(self.long_stop_list[0])
					"""
				...
				if (self.pos==0
					...
					if(	#Timeframe 0 signals
						...
						if not self.modelp.get('live_status'):
							...
							long_stop_name = '{} - Submit STOP for Long Entry'.format(d._name)
							...
							self.inorder_dict[d._name].append(self.long_ord)
					...
					#DEFINE SHORT ENTRY LOGIC
							...
							if not self.modelp.get('live_status'):
								...
								print(self.inorder_dict.get(d._name)[-1][1])
								"""
								self.inorder_dict[d._name].append(self.short_ord)
