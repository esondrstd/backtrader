...
from backtrader.feeds import mysql_fund
...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2018,4,25), #Dates for backtesting
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		#print(f'{self.dt} {self.hourmin} {self.tick_close[0]}')
		...
		#-----------------------------------------------------------------------------------------------------
		#print(f"{self.data._name[:-4]+'fund'} Right Now")
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			print(f'{d} NEXT STATEMENT')
			if self.data._name != self.data._name[:-4]+'fund':	
				print(f'{d} INSIDE LOOP WITHOUT FUND DATA')
				...
				if self.pos and self.tick_close[0]<=-500 and d._name == d._name[:-1]+'0':
...
def data_backtest(cerebro,start_date,end_date,session_start,session_end):
	...
	for i,j in enumerate(datalist):
		...
		fund_data = mysql_fund.MySQLFund(dbHost = host,
								dbUser = user,
								dbPWD = password,
								dbName = database,
								table = fund_table,
		...
		cerebro.adddata(fund_data, name=f'{j}fund')
