...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.pval_dict = defaultdict(lambda: defaultdict(list))
...
#**************************************************************************************************************************************
	...
	def calc_spread_zscore(self,z_entry_threshold=2.0):
		...
		for i in self.adfpval:			
			...
			net_pair_val = round((t1_data[-1]*hratio_t1) + (t2_data[-1]*hratio_t2),2)  #think market price of "spread", just like price per share for any stock.  Hedge ratio sign indicates whether you are long or short that instrument (i.e. - hedge ratio means short, positive long)
			self.num_units = self.p.total_dollars_risked/net_pair_val  #this represents how many "shares" of the pair spread you can long or short
			size_t1 = int(self.num_units*hratio_t1)
			size_t2 = int(self.num_units*hratio_t2)
			...
			self.inorder_dict[name]['net_val'].append(net_pair_val)
			...
			if self.hourmin=='08:35' and self.long_pair_dict.get(f'long_sig {name}'):
				if hratio_t1 > 0:
					...
					self.inorder_dict[name][f'{t1} size'].append(size_t1)
				...
				if hratio_t2 > 0:
					...
					self.inorder_dict[name][f'{t2} size'].append(size_t2)
	...
	def pairs_exit(self,z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			hratio_t1 = i['hratio1']
			hratio_t2 = i['hratio2']
			t1_data = self.justclose_dict.get(t1)[0]
			t2_data = self.justclose_dict.get(t2)[0]
			...
			self.pair_zscore_dict[f'zscore {t1}/{t2}'].append(zscore_now)
			...
			if exit_signal or self.hourmin=='14:50':
				self.close(t1)
				self.close(t2)
