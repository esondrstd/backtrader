...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['TICK-NYSE','SPY','XLU','AAPL','ABBV', 'ABT', 'ACN', 'ADBE']
...
class Strategy(bt.Strategy):
	...
	def notify_order(self, order):
		...
		if order.status == order.Completed:
			if order.isbuy():
				print('-'*80)
				print(f"BUY EXECUTED, Date: {date} Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value),2}, Size {order.executed.size}, Name: {order.info['name']}")
			
			elif order.issell():
				print('-'*80)
				print(f"SELL EXECUTED, Date: {date} Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value,2)}, Size {order.executed.size}, Name: {order.info['name']}")
	...
	def notify_trade(self, trade):
		...
		if trade.isclosed:
			print('-'*80)
			print(f"{date}, TRADE CLOSED - Price: {round(trade.price,2)}, Net Profit: {round(trade.pnlcomm,2)}")
	...
	def next(self):
		...
		#Stock Selection - once data is collected at 8:30, sort it
		if self.hourmin=='08:35' and self.gap_dict is not None:
			sorted_topres = sorted(self.gap_dict.items(), key = lambda x: x[1], reverse=True)  #Create sorted list -  key accepts a function (lambda), and every item (x) will be passed to the function individually, and return a value x[1] by which it will be sorted.
			sorted_botres = sorted(self.gap_dict.items(), key = lambda x: x[1])
			self.rtop_dict = dict(sorted_topres[:self.p.rank])  #Choose subset of tickers with highest rank (i.e. top 3)
			self.rbot_dict = dict(sorted_botres[:self.p.rank])  #Choose subset of tickers with lowest rank (i.e. bottom 3)
			...
			self.merged_topsorted_dict = dict(self.topsorted_dict) #Assign topsorted_tickers to new variable so merge can be done, yet top sorted dict can be retained for later use
			self.merged_botsorted_dict = dict(self.botsorted_dict)
			self.merged_topsorted_dict.update(self.merged_botsorted_dict) #merge the two lists and assign to new combined dict
			self.combosorted_dict = dict(self.merged_topsorted_dict)
			print(self.combosorted_dict.keys())
			self.sortflag = 1
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if d._name == 'TICK-NYSE0':
				self.tick_close = self.inds.get(d._name).get('ohlc').lines.c[0]
			
			if d._name == 'SPY0':
				self.spy_close = self.inds.get(d._name).get('ohlc').lines.c[0]
		
			#Stock Selection - collect data to be ranked at start of trading day and put in dictionary
			if self.hourmin=='08:30' and d._name!='TICK-NYSE0' and d._name==self.name_t0:
				#Determine open gap
				self.gap_dict[d._name] = round(self.inds.get(d._name).get('gap').lines.gap[0],3)
			
			if d._name == self.name_t0:
			#if d._name==self.name_t0 and d._name in self.combosorted_dict.keys() and self.combosorted_dict is not None:
				#print("Post Loop",self.dt, self.hourmin,d._name,self.pos)
				#print(d._name, self.dt, self.hourmin, self.getposition(d).size)
				#print(self.getpositionbyname(d._name).size)
				...
				#Calculate Sizing
				self.size = int(self.inds.get(d._name).get('atr_stop').lines.size[0])
				#print(d._name, self.dt, self.hourmin, self.long_stop,self.short_stop,self.size)

				#Calculate Target Prices
				self.target_short = self.target_short_dict.get(d._name)
				self.target_long = self.target_long_dict.get(d._name)
				...
				"""
				if self.long_stop_dict[d._name]:
					print(self.long_stop_dict[d._name],self.long_stop_dict[d._name][0])
				"""
				
				#If exiting trade via market order, cancel stop orders
				#print(d._name, self.dt,self.hourmin,self.orders[d._name])


				"""
				for o in self.orders:
					if o and o.info['name'] == f'{d._name} - EXIT LONG':
						self.cancel(self.long_stop_dict[d._name][-1])
						print(f'Cancel Stop Order for {d._name}')
				
					if o and o.info['name'] == f'{d._name} - EXIT SHORT' :
						self.cancel(self.short_stop_dict[d._name][-1])
						print(f'Cancel Stop Order for {d._name}')
				"""
				#print(d._name,self.dt,self.hourmin,self.pos)
				
#********************************EXIT LOGIC/ORDERS*********************************************
				#EXIT LOGIC FOR SHORTS
				if (d._name == self.name_t0 
					#and not (self.name_t0[:-1]=='VIX' or self.name_t0[:-1]=='TICK-NYSE' or self.name_t0[:-1]=='TRIN-NYSE' or self.name_t0[:-1]=='SPY')	
					and self.pos < 0
					#and (d.low[0] <= self.target_short)
					and self.target_short != 0
					and (self.low_t0 <= self.target_short or self.tick_close > 1000 or self.hourmin=='14:50')  
					and self.prenext_done #Start trading after all prenext data loads 
					):		


					print("Cancel Stops")
					for o in self.short_stop_ord[d._name]:
						cancel(o)
					
					#SHORT EXIT ORDER - closes existing position and cancels outstanding stop-loss ord	
					self.exit_short_name = f'{d._name} - EXIT SHORT'     
					self.exit_short = self.close(d._name,
												name=self.exit_short_name)
					
					
					self.orders[d._name].append(self.exit_short)
					#print(self.exit_short)
					
				
					#print('{} {} EXIT SHORT {} - {} shares at {}'.format(self.dt, self.hourmin, d._name,self.size_dict.get(d._name)[-1],self.close_t0))

				#EXIT LOGIC FOR LONGS
				elif (d._name == self.name_t0 
					#and not (self.name_t0[:-1]=='VIX' or self.name_t0[:-1]=='TICK-NYSE' or self.name_t0[:-1]=='TRIN-NYSE' or self.name_t0[:-1]=='SPY')	
					and self.pos > 0
					#and (d.high[0] >= self.target_long)
					and self.target_long != 0
					and (self.high_t0 >= self.target_long or self.tick_close < -1000 or self.hourmin=='14:50')
					and self.prenext_done  #Start trading after all prenext loads (live and backtest modes) 
					):
					
					#print('{} {} EXIT LONG {} - {} shares at {}'.format(self.dt, self.hourmin, d._name,self.size_dict.get(d._name)[-1],self.close_t0))
					print("Cancel Stops")
					for o in self.short_stop_ord[d._name]:
						cancel(o)
					
					#LONG EXIT ORDER - closes existing position and cancels outstanding stop-loss order
					self.exit_long_name = f'{d._name} - EXIT LONG' 
					
					self.exit_long = self.close(d._name,
												name=self.exit_long_name)
					
					self.orders[d._name].append(self.exit_long)
					#print(self.exit_long)
					#print("Cancel Stop Loss on trade exit")
					#self.cancel(self.long_stop_ord)				

				#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
				if (d._name==self.name_t0
					and not d._name[:-1]=='TICK-NYSE'
					and self.pos==0
					and (self.hourmin>='09:20'and self.hourmin<='11:00')
					...
					if(	#Timeframe 0 signals
						...
						if not self.modelp.get('live_status'):
							...
							self.target_long_price = self.open_t0+(self.p.dollars_risked_per_trade*self.p.target)/self.size
							self.target_long_dict[d._name] = self.target_long_price
							
							#Create Fixed Long Stop Loss
							long_stop_name = f'{d._name} - STOP'  #Add info when notification output triggers
							...
							long_name = f'{d._name} - BUY BUY BUY'
							self.long_ord = self.buy(data=d._name,
												size=self.size,
												exectype=bt.Order.Market,
												transmit=False,
							...
							#Create Fixed Long Stop Loss
							long_stop_name =  f'{d._name} - STOPPED OUT OF LONG TRADE'
							self.long_stop_ord = self.sell(data=d._name,
												size=self.size,
												exectype=bt.Order.Stop,
												price=self.long_stop,
					...
					#DEFINE SHORT ENTRY LOGIC
					elif (	self.pos==0  #Need to repeat in case buy happened at same bar
							and self.cash_avail > self.p.total_dollars_risked  #need to repeat incase buy happened at same bar
							#Timeframe 0 signals
							#self.percK_t0 > 90
							...
							if not self.modelp.get('live_status'):
								...
								#print(self.short_ord)
								self.target_short_price = self.open_t0-(self.p.dollars_risked_per_trade*self.p.target)/self.size
								self.target_short_dict[d._name] = self.target_short_price
																	
								#Create Fixed Short Stop Loss	 
								short_stop_name =  f'{d._name} - STOP'
								...
								#print('**************SHORT SIGNAL**************')
								#Create Short Entry Order
								short_name = f'{d._name} - SELL, SELL, SELL'
								self.short_ord = self.sell(data=d._name,
													size=self.size,
													exectype=bt.Order.Market,
													transmit=False,
								...
								#Create Fixed Short Stop Loss	 
								short_stop_name = f'{d._name} - STOPPED OUT OF SHORT TRADE'
								self.short_stop_ord = self.buy(data=d._name,
									size=self.size,
									exectype=bt.Order.Stop,
									price=self.short_stop,
...
#************************************************************************************************************************************		
		...
		plot_start = modelp.get('start_date')
		...
		for i in range (0,len(results[0].datas),3):
			...
			cerebro.plot(start = plot_start, end = plot_end, barup='olive', bardown='lightpink',volup = 'lightgreen',voldown='crimson')
