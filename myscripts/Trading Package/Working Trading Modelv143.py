...
#**************************************************************************************************************************************
		...
		#Loop through all pairs, run Johansen test, and pick out cointegrated stocks
			for i, (ticker1, ticker2) in enumerate(self.all_pairs): 	
				...
				if np.all(result.lr1 >= trace_crit_value) and np.all(result.lr2 >= eigen_crit_value):			
					...
					#print(ticker1,t1_data[-1],hedge_ratio_t1,ticker2,t2_data[-1],hedge_ratio_t2)
					"""
	...
	def calc_spread_zscore(self):
		...
		for i in self.adfpval:		
			...
			spread = np.subtract(np.array(t1_data),np.array(t2_data))
	...
	def pairs_entry_exit(self,z_entry_threshold=2.0, z_exit_threshold=1):
		...
		for i in self.adfpval:
			...
			net_pair_val = round((t1_data*hratio_t1) + (t2_data*hratio_t2),2)  #think market price of "spread", just like price per share for any stock.  Hedge ratio sign indicates whether you are long or short that instrument (i.e. - hedge ratio means short, positive long)
			...
			#print(zscore_now,z_entry_threshold, self.inorder_dict[-1],long_signal, short_signal, cash_avail, t1,size_t1,t2,size_t2)
			...
			if not self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and (long_signal or short_signal) and self.hourmin<'14:00' and cash_avail > self.p.total_dollars_risked:
				if hratio_t1 > 0:
			...
			if not self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and (long_signal or short_signal) and self.hourmin<'14:00' and self.cash_avail > self.p.total_dollars_risked:
				if hratio_t1 < 0:
