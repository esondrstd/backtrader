...
#**************************************************************************************************************************************
	...
	def create_pairs(self,d,signif):
		...
		for i, (ticker1, ticker2) in enumerate(all_pairs): 	
			...
			self.net_pair_val = (t1_data[-1]*self.hedge_ratio_t1) + (t2_data[-1]*self.hedge_ratio_t2)  #think market price of "spread", just like price per share for any stock.  Hedge ratio sign indicates whether you are long or short that instrument (i.e. - hedge ratio means short, positive long)
			...
			self.size_t1 = int(self.num_units*self.hedge_ratio_t1)
			self.size_t2 = int(self.num_units*self.hedge_ratio_t2)
			...
			if np.all(result.lr1 >= trace_crit_value) and np.all(result.lr2 >= eigen_crit_value):			
				...
				self.cointegrating_pairs.append(dict(t1=ticker1,t2=ticker2,hratio1 = self.hedge_ratio_t1,hratio2 = self.hedge_ratio_t2, ))
				print(self.cointegrating_pairs)
	...
	def pair_regression(self):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			self.adfuller_test(error,stock1,stock2,self.p.signif)
	...
	def calc_spread_zscore(self,z_entry_threshold=2.0,z_exit_threshold=1):
		
		for i in self.adfpval:
			t1 = i['t1'] 	#Y Variable
			t2 = i['t2']	#X Variable
			
			arrt1 = np.multiply(self.hedge_ratio_t1,np.array(self.justclose_dict.get(t1))) #create array of hedge ratio * close price
			arrt2 = np.multiply(self.hedge_ratio_t2,np.array(self.justclose_dict.get(t2))) 
			spread = np.subtract(arrt1,arrt2)
			...
			self.exit_pair_dict[f'exit_sig {t1}/{t2}'].append(zscore.any() <= z_exit_threshold*1.0)	
	...
	def pairs_entry(self,hratio1,hratio2):
		...
		for i in self.adfpval:
			...
			for i in mylist:
				...
				if self.hourmin=='08:35' and self.long_pair_dict.get(f'long_sig {name}' and h > 0):
					...
					short_name = f'{t2} - Enter Short PAIRS Trade'
					self.short_ord = self.sell(data=t2,
					size= self.size_t2,
					exectype=bt.Order.Market,
					transmit=True)
					
					print(f"{self.dt} {self.hourmin} {name} - long signal: {self.long_pair_dict.get(f'long_sig {name}')}")
			
				if self.hourmin=='08:35' and self.short_pair_dict.get(f'short_sig {name}'):
					#and not pos_t1 and not pos_t2 and self.cash_avail > self.p.total_dollars_risked:
					#Short stock 1 of pair
					short_name = f'{t1} - Enter Short PAIRS Trade'
					...
					long_name = f'{t2} - Enter Short PAIRS Trade'
