...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','AAPL','INTC','ADSK','ABBV', 'ABT', 'ACN', 'ADBE', 'ADI', 'ADP']
...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.rtop_dict = dict()
		self.rbot_dict = dict()
		self.spyrtop_dict = dict()
		self.spyrbot_dict = dict()
		self.topsorted_dict = dict()
		self.botsorted_dict = dict()
		self.spytopsorted_dict = dict()
		self.spybotsorted_dict = dict()
		self.close_dict = defaultdict(list)
		self.newdf = pd.DataFrame()
		self.correl_dict = defaultdict(list)
		self.ranked_dict = dict()
		self.spycorr_dict = defaultdict(list)
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.newdf is not None and self.hourmin=='08:35':
			self.rank_correl()
		
		if self.hourmin=='08:45':
			print("SPY CORRELATIONS", self.spycorr_dict)
			print("Ranked SPY Correlations",self.spyrtop_dict,self.spyrbot_dict)
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if (self.cor_counter  < self.p.correl*3) and d._name==d._name[:-1]+'0':  #prevent startup errors from missing data
				self.cor_counter += 1
			else:
				if self.hourmin == '08:30'  and d._name==d._name[:-1]+'0' and not d._name[:-1]=='TICK-NYSE':
					self.spycorr_dict.clear()
					self.closes= d.close.get(0,self.p.correl)
					self.rets = np.diff(np.log(self.closes)).round(4)
					self.close_dict[d._name] = [x for x in self.rets]	
					#self.newdf = pd.DataFrame.from_dict(self.close_dict)									
					self.newdf = pd.DataFrame.from_dict(self.close_dict)
					
					
			
			if self.hourmin == '08:40' and d._name==d._name[:-1]+'0' and not d._name=='SPY0' and not d._name[:-1]=='TICK-NYSE' and not self.newdf.empty:	
				self.spy_correl(d)
	...
	def rank_correl(self):
		"""Returns most highly correlated pairs of stocks, and correlation value, from ticker list via 2 key, 1 value dict"""
		#mycorr = round(self.newdf.corr(method='pearson'),2)
		np_matrix = self.newdf.to_numpy()
		print(self.newdf)
		mycorr = np.corrcoef(np_matrix.T)
		#mycorr = np.corrcoef(self.newdf.T)
		print(mycorr)
		#np.fill_diagonal(mycorr.values, np.nan)  #replace 1's with NA's in correlations matrix
		#mycorr.abs() #to get absolute values
		#correl = mycorr.unstack().sort_values(kind="quicksort",ascending=False).dropna().drop_duplicates()
		#correl = mycorr.unstack().dropna().drop_duplicates()
		correl = mycorr.unstack().sort_values(kind="quicksort",ascending=False).dropna()
		self.correl_dict = correl.to_dict()
		mysort = sorted(self.correl_dict.items(), key = lambda x: x[1], reverse=True)
		self.top_ranked_dict = dict(mysort[:self.p.rank]) 
		self.bot_ranked_dict = dict(mysort[-self.p.rank:]) 
		print("From Rank_Correl",self.top_ranked_dict,self.bot_ranked_dict)
	
	
	def spy_correl(self,d):	
		"""Returns correlations to SPY"""	
		alias = ('SPY0',d._name) #create one dict key out of two, and only get correlations to SPY.  Can be ordered either way, so need or
		newalias = (d._name,'SPY0') #provide alternative key if alias key not found
		#print(f'Correlation between {d._name} and SPY is {self.correl_dict.get(alias,self.correl_dict.get(newalias))}')  #can provide alternative value in 'get' if key not found
		self.spycorr_dict[d._name].append(self.correl_dict.get(alias,self.correl_dict.get(newalias)))
		mysort = sorted(self.spycorr_dict.items(), key = lambda x:x[1], reverse=True)
		self.spyrtop_dict = dict(mysort[:self.p.rank])
		self.spyrbot_dict = dict(mysort[-self.p.rank:]) 
		#print("YO YO YO",self.rtop_dict,self.rbot_dict)
		#self.spytopsorted_dict = {d: v for d,v in sorted(self.spyrtop_dict.items(),key=lambda x:[1],reverse=True)}  #Same as above, except converted to dictionary so we can extract ticker 'keys'.  d._name is the key, and v is the value .  Reverse = true sorts greatest first
		#self.spybotsorted_dict = {d: v for d,v in sorted(self.spyrbot_dict.items(),key=lambda x:[1])}  #Same as above, except converted to dictionary so we can extract ticker 'keys'.  d._name is the key, and v is the value .  Reverse = true sorts greatest first
		#print("Appended Dictionary",self.spytopsorted_dict,self.spybotsorted_dict)
