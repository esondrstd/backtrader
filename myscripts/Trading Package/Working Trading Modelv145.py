...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.long_stop_dict = defaultdict(lambda: defaultdict(list))
		self.short_stop_dict = defaultdict(lambda: defaultdict(list))
...
#**************************************************************************************************************************************
	...
	def calc_spread_zscore(self):
		...
		for i in self.adfpval:		
			...
			self.long_stop_dict[name][t1].append(0)
			self.long_stop_dict[name][t2].append(0)
			self.short_stop_dict[name][t1].append(0)
			self.short_stop_dict[name][t2].append(0)
	...
	def pairs_entry_exit(self,z_entry_threshold=2.0, z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			t1_long_stop_sig = t1_data < self.long_stop_dict.get(name).get(t1)[-1]
			t2_long_stop_sig = t2_data < self.long_stop_dict.get(name).get(t2)[-1]
			t1_short_stop_sig = t1_data > self.short_stop_dict.get(name).get(t1)[-1]
			t2_short_stop_sig = t2_data > self.short_stop_dict.get(name).get(t2)[-1]
			
			print(t1_long_stop_sig,t2_long_stop_sig,t1_short_stop_sig,t2_short_stop_sig)
			print(self.long_stop_dict.get(name).get(t1),self.long_stop_dict.get(name).get(t2),self.short_stop_dict.get(name).get(t1),self.short_stop_dict.get(name).get(t2))
			...
			if total_entry_signals !=0 and not self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and self.hourmin<'11:00' and cash_avail > self.p.total_dollars_risked:
				...
				if weights_t1>0:
					...
					self.long_stop_dict[name][t1].append(self.inds.get(t1).get('atr_stop').lines.long_stop[0])
				...
				if weights_t2>0:
					...
					self.long_stop_dict[name][t2].append(self.inds.get(t2).get('atr_stop').lines.long_stop[0])
				...
				if weights_t1<0:
					...
					transmit=True)
					self.size_dict[name][t1].append(size_t1)
					self.short_stop_dict[name][t1].append(self.inds.get(t1).get('atr_stop').lines.short_stop[0])
				...
				if weights_t2<0:
					...
					transmit=True)
					self.size_dict[name][t2].append(size_t2)
					self.short_stop_dict[name][t1].append(self.inds.get(t2).get('atr_stop').lines.short_stop[0])
