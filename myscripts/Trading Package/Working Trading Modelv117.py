...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if not self.newdf.empty and self.hourmin=='08:35':
			self.rank_correl()
	...
	def rank_correl(self):
		...
		print(mycorr["SPY0"].sort_values(ascending=False))

		#mycorr.abs() #to get absolute values
		correl = mycorr.unstack().sort_values(kind="quicksort",ascending=False).dropna().drop_duplicates()  #convert dataframe records from list to values so they can be passed to dictionary and accessed
