...
class Strategy(bt.Strategy):
	...
	params = dict(
			...
			ema1 = 5,  #8
			ema2 = 10, #20
			signif = .05, #for statistical tests: .01 ideal
			correl = 30,
			obv = 10,
			atrper = 5,
			atrdist = 2,   
	...
	def __init__(self):
		...
		self.inds = dict()
		self.boll_dict = defaultdict(list)
		...
		self.appended_dict = defaultdict(list)
		self.add_df = pd.DataFrame()
		self.datedf = pd.DataFrame()
...
#*********************************************INITITIALIZE INDICATORS*********************************************************				
			self.inds[d._name]['close'] = btind.close(d.close,period=self.p.correl,plot=False)	#history of closing prices
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hourmin=='08:35' and not self.df_closes.empty:
			self.clear_pairs()	#deletes dataframe
			#Pairs Trading Strategy
			self.create_pairs(self.df_closes,self.p.signif)  #get initial pairs list using Johansen test
			self.pair_regression(self.df_closes)  #refine pairs list by performing ols regression than ADF test
			self.calc_spread_zscore(self.df_closes)
			self.pairs_signal(self.df_closes)
			#self.plotdf(self.df_closes,self.adfpval) #Plot final pairs
			
		if not self.df_closes.empty:
			self.update_df(self.data) 	#Add row to dataframe
			self.appended_dict.clear()	#clear dictionary for memory savings - only 1 row needed
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if  d._name==d._name[:-1]+'1' and d._name!='TICK-NYSE1' and not self.df_closes.empty:
				self.appended_dict[d._name].append(d.close[0])
	...
	def clear_pairs(self):
		self.df_closes.iloc[0:0] 
		self.newdf.iloc[0:0]
		self.df_dates.iloc[0:0]
		self.add_df.iloc[0:0]
		self.datedf.iloc[0:0]
		self.adfpval.clear()
	...
	def pairs_signal(self,df,z_entry_threshold=1.0,z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			df['longs'] = (df[f'zscore {t1}/{t2}'].values <= -z_entry_threshold)*1.0
			df['shorts'] = (df[f'zscore {t1}/{t2}'].values >= z_entry_threshold)*1.0
			df['exits'] = (np.abs(df[f'zscore {t1}/{t2}'].values) <= z_exit_threshold)*1.0
				
		for k,vals in self.pair_zscore_dict.items():
			name = k[7:]
			for i in vals:
				for v in i:
					self.long_pair_dict[f'long_sig {name}'].append(v <= -z_entry_threshold*1.0)
					self.short_pair_dict[f'short_sig {name}'].append(v >= z_entry_threshold*1.0)
					self.exit_pair_dict[f'exit_sig {name}'].append(v <= z_exit_threshold*1.0)
		print(self.long_pair_dict)
		print(self.short_pair_dict)
		print(self.exit_pair_dict)

		self.zscore_added = 1  #flag to indicate function was run
	
	
	def calc_spread_zscore(self,df):
		...
		for i in self.adfpval:
			...
			t2 = i['t2']
			hratio = i['hratio']
			#print("HEDGE RATIO FROM OLS Coefficient",t1,t2,hratio)
		
			#add spread to dataframe
			df[f'spread {t1}/{t2}'] = df[t1].values-hratio*df[t2].values #Add spread to dataframe for zscore calculations. Beta neutral hedge ratio balances volatility(beta) in each stock.  Beta neutral simply mean the book is neutral on a beta level and doesn't have to be neutral in terms of market exposure (eg, long $50mm of VRX with beta 2.0 and short $100mm of ZTS with a beta of 1. your beta exposure is 0).  Market Neutral would mean being long and short the same nominal exposure (eg, long $100mm of AAPL, and short $100mm worth of GOOG. Your overall market exposure is $0)
			
			#Create z-score of the spread
			df[f'zscore {t1}/{t2}'] = (df[f'spread {t1}/{t2}'].values - np.mean(df[f'spread {t1}/{t2}'].values))/np.std(df[f'spread {t1}/{t2}'].values)
			
			self.hratio_close_dict[t2] = [hratio*x for x in self.justclose_dict.get(t2)]  #mutiply stock 2 array by hratio
				
			zip_obj = zip(self.justclose_dict.get(t1), self.hratio_close_dict.get(t2))
			for list1_i, list2_i in zip_obj:  #subtract lists to get spread
				myspread = list1_i-list2_i
				spread = myspread[0]
				print("THE SPREAD",spread)
				self.pair_spread_dict[f'spread {t1}/{t2}'].append(spread[0])
				
		for i in self.pair_spread_dict:
			name = i[7:]
			zscore = stats.zscore(self.pair_spread_dict[i])
			self.pair_zscore_dict[f'zscore {name}'].append(zscore)
				
		print("FROM ZSCORE FUNC",self.pair_spread_dict)
		print("FROM ZSCORE FUNC",self.pair_zscore_dict)
		
		
	def pair_regression(self,df):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			#stock2_open = df[stock2].iloc[-1]
			
			print(f'Stock 1: {stock1} of pair {i}: {stock1}/{stock2} Hedge ratio: {stock1_hratio}')
			print(f'Stock 2: {stock2} of pair {i}: {stock1}/{stock2} Hedge ratio: {stock2_hratio}')
			
			#Perform regression on pairs
			Y = df[stock1].values
			beta = df[stock2].values
			alpha = sm.add_constant(beta)
			
			regression = sm.OLS(endog=Y, exog=beta)
			results = regression.fit()
			hedgeratio = results.params
			#print(results.summary())
			
			#Perform ADF test on pairs
			self.adfuller_test(results,stock1,stock2,hedgeratio,self.p.signif)
			
		print("NUMBER OF PASSING ADF PAIRS",len(self.adfpval))		
	
	
	def create_pairs(self,df,signif):
		"""
		...
		#all_tickers = list(df.columns.values)  #return list of all tickers
		...
		for i, (ticker1, ticker2) in enumerate(all_pairs): 	
			#print(f"Running Johansen test on pair {i} of {len(all_pairs)}")  #Display progress
			pair_closes = df[[ticker1, ticker2]].values

			# The second and third parameters indicate constant term, with a lag of 1. 
			result = coint_johansen(pair_closes, 0, 1)
			hedge_ratio = list(result.evec[0])	#The first column of eigenvectors contains the best weights.  i.e. for every unit of XOM, we hold an opposite position in CVX at a ratio of 0.47
			...
			if np.all(result.lr1 >= trace_crit_value) and np.all(result.lr2 >= eigen_crit_value):
				print(f'Johansen: Pair:{ticker1} {ticker2},{ticker1} - Trace Stat: {t1_lr1} is > Crit Val {t1_trace} Max Eigen Stat {t1_lr2} > {t1_eigen} Crit Val, Hedge: {hedge_ratio[0]}')
				print(f'Johansen: Pair:{ticker1} {ticker2},{ticker2} - Trace Stat: {t2_lr1} is > Crit Val {t2_trace} Max Eigen Stat {t2_lr2} > {t2_eigen} Crit Val, Hedge: {hedge_ratio[1]}')

				self.cointegrating_pairs.append(dict(t1=ticker1,t2=ticker2,hratio_t1=hedge_ratio[0],hratio_t2=hedge_ratio[1]))
		print("NUMBER OF COINTEGRATING PAIRS:", len(self.cointegrating_pairs))  #return number of cointegrating pairs

	
	def update_df(self,d):
		if not self.df_closes.empty and self.newdf.empty:
			self.newdf = self.df_closes
		elif not self.newdf.empty:
			self.add_df = pd.DataFrame(self.appended_dict,index=[d.num2date()])
			self.newdf = self.newdf.append(self.add_df)	
			
		#print(self.newdf)

			
	def create_df(self,d):
		#Create dataframe of returns that can be used for cointegration calculations
		self.closes = self.inds.get(d._name[:-1]+'1').get('close').get(size=self.p.correl)

		self.dates = d.datetime.get(0,self.p.correl)
		self.justclose_dict[d._name] = [x for x in self.closes]	
		#print(self.justclose_dict)
		...
		if d._name==self.data1._name:  #Just need date for 1 symbol
			...
			self.df_dates = pd.DataFrame.from_dict(self.justdate_dict)
			self.df_dates.rename(columns={self.df_dates.columns[0]: "date" }, inplace = True)
			self.df_dates.set_index('date', inplace=True)
			
		self.df_closes = pd.DataFrame.from_dict(self.justclose_dict)
		self.df_closes.set_index(self.df_dates.index,inplace=True)  #make date index of close price dataframe
		#print(self.df_closes)
	
		"""
		rets = np.diff(np.log(self.closes)).round(4)
		self.close_dict[d._name] = [x for x in rets]								
		self.df_returns = pd.DataFrame.from_dict(self.close_dict)
		#print("Dataframe of Returns",self.df_returns)
		"""
	
	
	def plotdf(self,df,mydict):
		for i, pair in enumerate(mydict):
			t1_name = pair.get('t1')
			t2_name = pair.get('t2')
			ticker1 = df[pair.get('t1')]
			ticker2 = df[pair.get('t2')]

			plot1 = ticker1.plot.line(legend=True,title=f'Historical Close Prices for {t1_name} and {t2_name}')
			plot2 = ticker2.plot.line(secondary_y=True,legend=True)
			plot1.set_ylabel(t1_name)
			plot2.set_ylabel(t2_name)
			plt.show()
	
	
	def adfuller_test(self,ols_results,t1_name,t2_name,hratio,signif):
		...
		series = ols_results.resid  #get residuals of results from OLS
		...
		if p_value <= signif:
			...
			self.adfpval.append(dict(t1=t1_name,t2=t2_name,hratio=hratio))
		else:
