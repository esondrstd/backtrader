...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if not self.newdf.empty:
			...
			sort_correl = mycorr.unstack().sort_values(kind="quicksort",ascending=False).dropna()
			self.correl_dict = sort_correl.to_dict()
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if d._name==d._name[:-1]+'0' and not self.newdf.empty and not d._name=='SPY0' and not d._name[:-1]=='TICK-NYSE':
				alias = ('SPY0',d._name) #create one dict key out of two, and only get correlations to SPY
				print(d._name,self.correl_dict[alias])
