...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2019,10,1), #Dates for backtesting
...
class Strategy(bt.Strategy):
	...
	params = dict(
			...
			ema1 = 5,  #8
			ema2 = 10, #20
			z_entry = 3,  #significance level required to enter pairs trade
...
#**************************************************************************************************************************************
	...
	def initialize_pairs(self):
		...
		for i, d in enumerate(self.datas):
			if d._name==d._name[:-1]+'0'and d._name[:-1] != 'TICK-NYSE':
				self.all_tickers.append(d._name)
	...
	def pairs_entry_exit(self,z_entry_threshold, z_exit_threshold=.1):
		...
		for i in self.adfpval:
			...
			#print(self.dt,name,self.zscore_dict,self.zscore_dict.get(f'{name}')[-1])
			#print(self.dt,name,zscore_now, self.zscore_dict.get(f'{name}')[-1])
			total_entry_signals = long_signal + short_signal
			
			if self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and (exit_signal or stop_signal or (self.hour==14 and self.minute==45)):
				...
				if exit_signal:
					print(f"PROFIT TARGET HIT!!! - EXIT pair {t1}/{t2} zscore: {zscore_now}")
				
				if stop_signal:
					print(f"{self.dt} {t1}/{t2}**********STOPPED OUT********** zscore: {zscore_now} {self.zscore_dict.get(f'{name}')[-1]+.3}")
				
				self.inorder_dict[f'{t1}/{t2} inorder'].append(False)
			
			#print(cash_avail, self.pos)
			...
			if total_entry_signals !=0 and not self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and self.trade_end and cash_avail > self.p.total_dollars_risked:
				...
				if weights_t1>0:
					...
					print(f"{self.dt} {self.hour} {self.minute} For Pair {name} - Enter LONG Trade for leg {t1} at price {t1_data} zscore: {zscore_now} {stats.zscore(self.pair_spread_dict.get(f'spread {t1}/{t2}')[-self.p.pairs_lookback:])[-1]}")
				if weights_t2>0:
					...
					print(f"{self.dt} {self.hour} {self.minute} For Pair {name} - Enter LONG Trade for leg {t2} at price {t2_data} zscore: {zscore_now} {stats.zscore(self.pair_spread_dict.get(f'spread {t1}/{t2}')[-self.p.pairs_lookback:])[-1]}")
				if weights_t1<0:
					...
					transmit=True)
					self.size_dict[name][t1].append(size_t1)
					print(f"{self.dt} {self.hour} {self.minute} For Pair {name} - Enter SHORT Trade for leg {t1} at price {t1_data} zscore: {zscore_now} {stats.zscore(self.pair_spread_dict.get(f'spread {t1}/{t2}')[-self.p.pairs_lookback:])[-1]}")
				if weights_t2<0:
					...
					transmit=True)
					self.size_dict[name][t2].append(size_t2)
					print(f"{self.dt} {self.hour} {self.minute}  For Pair {name} - Enter Short Trade for leg {t2} at price {t2_data} zscore: {zscore_now} {stats.zscore(self.pair_spread_dict.get(f'spread {t1}/{t2}')[-self.p.pairs_lookback:])[-1]}")
				
				
				self.inorder_dict[f'{name}'].append(True)	
				self.zscore_dict[f'{name}'].append(abs(zscore_now)+.3)
