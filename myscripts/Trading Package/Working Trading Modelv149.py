...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		#if self.hourmin=='08:35' and self.gap_dict is not None and self.data._name==self.data0._name: 
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if d._name==d._name[:-1]+'0' and self.pos==0 and self.entry_rules(d):  #Check that entry rules are met: 		
				...
				#BUY SIGNAL
				if self.tick_trade('long') and d._name=='SPY0':
				#if self.regime_early_bull(d) and self.sup_res(d,'long'):
				#if self.hourmin=='08:30' and self.eps(d,'long'):
				#if self.sup_res(d,'long'):
				#if self.pull_back(d,'long'):
					self.buyorder(d)
				
				#SELL SIGNAL
				if self.tick_trade('short') and d._name=='SPY0':
				#if self.regime_early_bear(d) and self.sup_res(d,'short'):
				#if self.hourmin=='08:30' and self.eps(d,'short'):
				#if self.sup_res(d,'short'):
				#if self.pull_back(d,'short'):
					self.sellorder(d)
	...
	def eps(self,d,direction):
		if d._name[:-1] in self.eps_dict.keys() and str(self.dt) in self.eps_dict.get(d._name[:-1]).get('date'):
			if self.eps_dict.get(d._name[:-1]).get('eps_diff%') > 0 and direction=='long':
				return True
			elif self.eps_dict.get(d._name[:-1]).get('eps_diff%') < 0 and direction=='short':
				return True
			else:
				return False
		else:
	...
	def tick_trade(self,direction):
		if self.tick_close[0] >= 500 and direction=='short':
			return True
		elif self.tick_close[0] <= -500 and direction=='long':
			return True
		else:
			return False
	...
	def pull_back(self,d,direction):
	...
	def mean_revert(self,d,direction):
		...
		if d.close[0] >= self.boll_top_t1 and direction=='long':
			return True
		elif d.close[0] <= 
	"""	
