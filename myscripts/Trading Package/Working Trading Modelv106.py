...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','XLK','A', 'AAL', 'AAPL', 'ABBV', 'ABT', 'ACN', 'ADBE']
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if d._name==self.name_t0 and d._name in self.combosorted_dict.keys() and self.combosorted_dict is not None:
				...
				self.target_short = round(self.open_t0-(self.p.dollars_risked_per_trade*self.p.target)/self.size,2)
				self.target_long = round(self.open_t0+(self.p.dollars_risked_per_trade*self.p.target)/self.size,2)
				...
				if self.hourmin=='14:50' and self.pos != 0:
				...
				if (d._name == self.name_t0 
					...
					and self.size_dict.get(d._name) is not None
					...
					print(f'{self.dt} {self.hourmin} EXIT SHORT {d._name} - {self.size_dict.get(d._name)[-1]} shares at {d.close[0]}')
					self.size_dict[d._name].append(0)
				...
				#EXIT LOGIC FOR INTRADAY LONGS
					...
					print(f'{self.dt} {self.hourmin} EXIT LONG {d._name} - {self.size_dict.get(d._name)[-1]} shares at {d.close[0]}')
				...
				if (self.pos==0
					...
					if(	#d._name in self.topsorted_dict.keys()
						#Timeframe 0 signals
						#d.close[0]>d.close[-1]
						...
						#Create Target for Long
						long_target_name = '{} - Submit TARGET for Long Entry'.format(d._name)
						self.long_target_ord = self.sell(data=d._name,
							size=self.size,
							exectype=bt.Order.Limit,
							price=self.target_long,
							transmit=True,
							parent=self.long_ord,
						...
						self.target_long_dict[d._name].append(self.long_target_ord)
						print(f'{self.dt} {self.hourmin} BUY BUY BUY {d._name} - {self.size} shares at {d.close[0]}.  Stop price @ {self.long_stop}')
					...
					#DEFINE SHORT ENTRY LOGIC
							...
							#and d._name in self.botsorted_dict.keys()
							#Timeframe 0 signals
							#self.percK_t0 > 90
							...
							short_stop_name = '{} - Submit STOP for Short Entry'.format(d._name)
							...
							#Create Target for Short 
							short_target_name = '{} - Submit TARGET for Short Entry'.format(d._name)
							self.short_target_ord = self.buy(data=d._name,
								size=self.size,
								exectype=bt.Order.Limit,
								price=self.target_short,
								transmit=True,
								parent=self.short_ord,
								name = short_target_name,
								)
									
							self.short_stop_dict[d._name].append(self.short_stop_ord)
							self.target_short_dict[d._name].append(self.short_target_ord)
							self.size_dict[d._name].append(-self.size)
							print(f'{self.dt} {self.hourmin} SELL SELL SELL {d._name} - {self.size} shares at {d.close[0]}.  Stop price @ {self.short_stop}')
							
								
							"""
							#FOR BRACKET TRADING
							short_name = f'{d._name} - Enter Short Trade, Stop price: {self.short_stop}, Target: {self.target_short}'
							self.short_ord = self.sell_bracket(data=d._name,size=self.size,limitprice=self.target_short,stopprice=self.short_stop,name=short_name)
							print(self.inorder_dict.get(d._name)[-1][1])
							"""
