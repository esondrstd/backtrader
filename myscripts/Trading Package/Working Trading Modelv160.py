...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ('TICK-NYSE','SPY','XLU','IAU')
	...
	def model_params():
		params = dict(
			...
			start_date = date(2020,3,1), #Dates for backtesting
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hour==8 and self.minute==55:
			self.clear_dicts()	
		...
		if self.hour==9 and self.minute==0 and self.data._name==self.data0._name and self.perc_chg_dict is not None:
		...
		for n in self.merged_dict.keys():
			...
			if self.pos:
				...
				if self.percK_t0 > 90 or self.tick_close[0]>800:
					self.exit_trade(d,'long')
				elif self.percK_t0 < 10 or self.tick_close[0]<=-800:
					self.exit_trade(d,'short')
			"""
			
			#---------------------------------ENTRY LOGIC FOR LONG AND SHORT-----------------------------------------
			if d._name==d._name[:-1]+'0' and self.pos==0 and self.hour in self.trade_hrs and d._name[:-1] != 'TICK-NYSE' and self.entry_rules(d):  #Check that entry rules are met
				#sig = self.sup_res(d)
