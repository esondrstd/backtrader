	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if d._name == d._name[:-1]+'0': #For a single timeframe only (timeframe 0)
				...
				#self.bullish_three_line_strike_pattern_t0= self.bullish_three_line_strike(self.open_t0,self.close_t0)
				#self.bearish_three_line_strike_pattern_t0= self.bearish_three_line_strike(self.open_t0,self.close_t0)
				#self.bullish_engulfing_pattern_t0= self.bullish_engulfing(self.open_t0,self.close_t0,self.slope_t0)
				#self.bearish_engulfing_pattern_t0= self.bearish_engulfing(self.open_t0,self.close_t0,self.slope_t0)
				...
				#Determine open gap and opening 15 min range
			...
			if d._name == d._name[:-1]+'1': #For a single timeframe only (timeframe 1)
				...
				#self.bullish_engulfing_pattern_t1= self.bullish_engulfing(self.open_t1,self.close_t1,self.slope_t1)
				#self.bearish_engulfing_pattern_t1= self.bearish_engulfing(self.open_t1,self.close_t1,self.slope_t1)	
				#self.bullish_three_line_strike_pattern_t1= self.bullish_three_line_strike(self.open_t1,self.close_t1)
				#self.bearish_three_line_strike_pattern_t1= self.bearish_three_line_strike(self.open_t1,self.close_t1)
			...
			print(d._name,self.open_t0,d.open[0],d.open[-1],round(self.inds.get(self.name_t0).get('ohlc').lines.open[-1],3))
			#print(self.dt,self.hourmin, d._name, self.cash_avail, self.pos, self.size_dict.get(d._name)[-1], self.inorder_dict[d._name], self.percK_t0, self.close_t0, self.target_long, self.target_short)
		
			#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
			if (
				d._name == d._name[:-1]+'0'  #Trade timeframe 1 only
				and self.cash_avail > pos_cost
			...
			#********************************EXIT LOGIC/ORDERS*********************************************
				...
				if (d._name == d._name[:-1]+'0' 
					...
					and self.prenext_done #Start trading after all prenext data loads 
					and (self.low_t0 <= self.target_short or self.hourmin=='14:45')  
					):		
				...
				#EXIT LOGIC FOR LONGS
					...
					and (self.high_t0 >= self.target_long or self.hourmin=='14:45')
					):
