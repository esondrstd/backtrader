...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2017, 1, 24), #Dates for backtesting
...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() >  self.max_ind_period():
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				if (self.inorder_dict[d._name] is True and self.order and self.pos is 0):  #if inorder and position changes to 0, update dictionaries to reflect inorder is false and size is back to 0
					self.inorder_dict[d._name].append(False)
					self.size_dict[d._name].append(0)
				
				print(d._name, dt, self.hourmin, cash_avail,self.pos,self.size_dict.get(d._name), self.inorder_dict.get(d._name), self.percK_t0,self.slope_t0,self.slope_t1,self.slope_t2, self.volume_t1,self.intraday_volume_t1,self.low_t1,self.support_t1,self.pbar_close_t1)
				
				#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
				if (cash_avail > pos_cost
					and self.size_dict.get(d._name)[0] is 0
					and self.inorder_dict.get(d._name)[0] is False
					and self.prenext_done #Start trading after all prenext data loads
					and (self.hourmin>='08:55'and self.hourmin<='14:30')
					):  	
					#DEFINE LONG ENTRY LOGIC
					if(	#Timeframe 0 signals
						self.percK_t0 < 30
						and self.slope_t0 < 0
						#self.volume_t0 > self.intraday_volume_t0
						#Timeframe 1 signals
						...
						if d._name == d._name[:-1]+'1':  #Trade timeframe 1 only
							...
							if not modelp.get('live_status'):
								...
								self.inorder_dict[d._name][0]=True
				...
				#********************************EXIT LOGIC/ORDERS*********************************************
					...
					if (d._name == d._name[:-1]+'1'
						...
						and self.size_dict.get(d._name)[0] < 0
						and self.inorder_dict.get(d._name)[0] is True
						and self.prenext_done #Start trading after all prenext data loads 
						and self.short_exit_price is not None
						and (self.low_t1 <= self.short_exit_price)
						):		
					...
					#EXIT LOGIC FOR LONGS
						...
						and self.long_exit_price is not None
						and (self.high_t1 >= self.long_exit_price)
						):
