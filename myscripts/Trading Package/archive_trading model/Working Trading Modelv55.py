...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			number_of_timeframes = 4, #**DEPENDENT ON "TIMEFRAMES ON" BELOW, MANUALLY ADJUST if timeframe set to off**
			base_timeframe = 15, #MINUTES
			timeframe0 = 5, #MINUTES
			timeframe1 = 15, #MINUTES
			timeframe2 = 60, #MINUTES
			timeframe3 = 1,#(Days) - must be days or resample screws up when calculting prior day high/low/close/volume
			timeframe1on = True,
			timeframe2on = True,
			timeframe3on = False,
			printlines = False,
	...
	def ind_params():
		params = dict(
			...
			ema1= 10,
			ema2= 20,
			atrperiod= 14,  #standard is 14
			atrdist= 2,   
			slope_period=14,
			breakout_per=3,
			hist=70, #Minimum needed to caclulate 10 day period volume
			avg_per=3,
			avg_volume=20,
			stoch_per=14,  #standard is 14
			stoch_fast=3,  #standard is 3
			adx_per = 14,  #standard is 14
			bollinger_period=20,  #standard is 20
			bollinger_dist=2,
			lookback=3,#Support/Resistance calcs
			)
		return params
...
class Strategy(bt.Strategy,UserInputs):
	...
	def __init__(self):
		...
		if not modelp.get('live_status'):
			...
			self.ticker_interval = int(data_feed_count/self.ticker_count) #Needs to be an integer
		...
		for i, d in enumerate(self.datas):
			...
			#Data History Indicator for High, Low, Close, and Volume
			self.inds[d._name]['close_hist'] = btind.Hist(d.close,
													period=indp.get('hist'),
													plot=False)
													
			self.inds[d._name]['high_hist'] = btind.Hist(d.high,
													period=indp.get('hist'),
													plot=False)
													
			self.inds[d._name]['low_hist'] = btind.Hist(d.low,
													period=indp.get('hist'),
													plot=False)
			
			self.inds[d._name]['volume_hist'] = btind.Hist(d.volume,
													period=indp.get('hist'),
													plot=False)
													
			if d._name == d._name[:-1]+'1':
				self.inds[d._name]['volume_hist_t1'] = btind.Hist(d.volume,
													period=indp.get('hist'),
													plot=False)
			...
			self.inds[d._name]['atr_intraday'] = btind.ATR(d,
											period=1,
											plot=False)						
	...
	def prenext(self):
		...
		for i, d in enumerate(self.datas):
			if len(self)>200:
				print('{}, prenext len {} - counter {}'.format(d._name,d.datetime.datetime().strftime('%Y-%m-%d %H:%M:%S'),len(self), self.counter))
		"""
		self.next()
	...
	def next(self):
		...
		#Counter for VWAP Calculation (# of cumulative 5 min time periods within 1 day)
...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() > self.max_ind_period():	#Need to ensure enough data is preloaded to do calculations below
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				if d._name==self.name_t0:  #only calculate VWAP for timeframe0 and ensure enough data is preloaded.  		
				...
				self.sma2_t1 = round(self.inds.get(self.name_t1).get('sma2')[0],3)
				...
				#Calculate Average Volume per intraday time period
				#5 MIN timeframe
				if d._name == d._name[:-1]+'0' and indp.get('hist')> ((390/modelp.get('timeframe2'))*10):  #ensure just for timeframe 0, and at least 65 periods load (1 every 6.5 hours for longest timeframe, or 10 times which is 65)
					self.volume_list_t0 = self.inds.get(self.name_t0).get('volume_hist').get(ago=0,size=780)  #Get last 10 periods (780/78)
					self.period_volume_list_t0 = self.volume_list_t0[::-78]  #start at most recent value, and step back at 78 unit interval 
					self.avg_volume_t0 = (sum(self.period_volume_list_t0))/10 
					#print(d._name, self.hourmin,d.volume[0],self.period_volume_list_t0,self.avg_volume_t0)
				
				#15MIN timeframe
				if d._name == d._name[:-1]+'1' and indp.get('hist')> ((390/modelp.get('timeframe2'))*10): #ensure just for timeframe 1, and at least 65 periods load (1 every 6.5 hours for longest timeframe, or 10 times which is 65)
					self.volume_list_t1 = self.inds.get(self.name_t1).get('volume_hist_t1').get(ago=0,size=260)  #Get last 10 periods (780/78)
					self.period_volume_list_t1 = self.volume_list_t1[::-26]  #start at most recent value, and step back at 78 unit interval 
					self.avg_volume_t1 = (sum(self.period_volume_list_t1))/10 
					#print(d._name, self.hourmin,d.volume[0],self.period_volume_list_t1,self.avg_volume_t1)
				
				#Calculate Average Volume over fixed lookback period
				self.avg_volume_t1 = self.inds.get(self.name_t1).get('avg_volume')[0]
				...
				#Calc average ATR by intraday period
				self.atr_list = self.inds.get(self.name_t1).get('atr_intraday').get(ago=0,size=780)  #Get last 10 periods (780/78)
				self.period_atr_list = self.atr_list[::-78] #start at most recent value, and step back at 78 unit interval 
				self.avg_atr = round((sum(self.period_atr_list))/10,3)
				#print(d._name, self.hourmin, self.period_atr_list,self.avg_atr)
				...
				if d._name == d._name[:-1]+'0':	#For a single timeframe only (timeframe 0)
					self.size = self.atrsize  #needs to be integer for IB to process .  #Ensure size doesn't cost more than total_dollars_risked
					
					#Calculate exit prices for base trading timeframe
					#if self.name_t1 in self.target_short_dict and self.name_t1 in self.target_long_dict and d._name==self.name_t1: 
				...
				if (
					...
					and (self.hourmin>='08:50'and self.hourmin<='14:00')
					):  	
					#DEFINE LONG ENTRY LOGIC
					if(	#self.slope_t1 > 0
						#self.bullish_engulfing_pattern_t1
						d.close[0] > 1
						#and d.volume[0] > self.avg_volume_t1 
						#self.slope_t1 > 0
						#and self.slope_t2 > 0
...
def runstrat():
	...
	if modelp.get('live_status'):
		...
		for i,j in enumerate(ibdatalist):
			...
			if modelp.get('timeframe3on'):
				data_Timeframe3 = cerebro.resampledata(data,name="{}3".format(j),
														timeframe=bt.TimeFrame.Minutes,
														compression = modelp.get('timeframe3'))							
		...
		for i,j in enumerate(datalist):
			...
			data_Timeframe0 = cerebro.adddata(data=data, 
												name="{}0".format(j),
												)
			data_Timeframe0.csv=True #Report this data to csv file (true) or not (false)	
			data_Timeframe0.plotinfo.plot = True
			...
			if modelp.get('timeframe1on'):
				...
				#data_Timeframe1.plotinfo.plotmaster = timeframe0
			...
			if modelp.get('timeframe2on'):
				...
				#data_Timeframe2.plotinfo.plotmaster = timeframe0
			
			if modelp.get('timeframe3on'):
				data_Timeframe3 = cerebro.resampledata(data,
										name="{}3".format(j),
										timeframe=bt.TimeFrame.Days,
										compression = modelp.get('timeframe3'),
										)
				data_Timeframe3.csv=False #Report this data to csv file (true) or not (false)	
				data_Timeframe3.plotinfo.plot = True
				#data_Timeframe3.plotinfo.plotmaster = timeframe0
