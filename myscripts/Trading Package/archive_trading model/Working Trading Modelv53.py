...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() >  self.max_ind_period():
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				self.highest_t3 = round(self.inds.get(self.name_t3).get('highest')[0],3) 
				self.lowest_t3 = round(self.inds.get(self.name_t3).get('lowest')[0],3) 

				#Calculate Prior Day High, Low, Close, Volume
				self.pday_close_t3 = round(self.inds.get(self.name_t3).get('close_hist')[0],3) 
				self.pday_high_t3 = round(self.inds.get(self.name_t3).get('high_hist')[0],3) 
				self.pday_low_t3 = round(self.inds.get(self.name_t3).get('low_hist')[0],3) 
				self.pday_volume_t3 = round(self.inds.get(self.name_t3).get('volume_hist')[0],3)
				
				#Calculate Average Volume per intraday time period
				self.volume_list = self.inds.get(self.name_t0).get('volume_hist').get(ago=0,size=780)  #Get last 10 periods (780/78)
				self.period_volume_list = self.volume_list[::-78]  #start at most recent value, and step back at 78 unit interval 
				self.avg_volume = (sum(self.period_volume_list))/10 
				#print(d._name, self.hourmin,d.volume[0],self.period_volume_list,self.avg_volume)
				
				#Calculate Average Volume over fixed lookback period
				self.avg_volume_t3 = self.inds.get(self.name_t1).get('avg_volume')[0]
				...
				self.atr_list = self.inds.get(self.name_t0).get('atr_intraday').get(ago=0,size=780)  #Get last 10 periods (780/78)
				...
				#Calculate ATR Stop Loss Distance (to be subtracted from price for stop loss calculations.  Determine atr timeframe based on user defined parameter.  EVAL turns a string into a variable that can return a value
				...
				if mycount > (len(self.datas)):  #dic count is greater than # of tickers * # of data timeframes which equals all "datas"
	...
	def maxtimeframe(self):
		...
		time3 = UserInputs.model_params().get('timeframe3on')
		if time1 and time2:
	...
	if modelp.get('live_status'):
		...
		for i,j in enumerate(datalist):
			...
			if modelp.get('timeframe3on'):
				...
				#data_Timeframe3.plotinfo.plotmaster = data_BaseTimeframe
