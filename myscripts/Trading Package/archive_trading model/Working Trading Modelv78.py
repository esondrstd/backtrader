...
class Strategy(bt.Strategy):
	params = dict(
			...
			tolerance_perc=2,#Support/Resistance - % of total max-min move range (area where program starts recording touches to levels)
			bounce_perc=5,#Support/Resistance - % of total max-min move range
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			#print(d._name, self.hourmin,d.close[0],d.close[-1],d.volume[0],self.obv_t0,self.slope_obv_t0)
			...
			#print(d._name, self.dt, self.hourmin, d.high[0],self.resistance_t1,d.low[0],self.support_t1,self.slope_t1,self.slope_t2,self.slope_obv_t0)
			...
			#********************************EXIT LOGIC/ORDERS*********************************************
				...
				if (d._name == d._name[:-1]+'0' 
					...
					and (d.low[0] <= self.target_short or self.cross_t0== 1.0 or self.tick_close>1000 or self.hourmin=='14:50')  
					):		
				...
				#EXIT LOGIC FOR LONGS
					...
					and (d.high[0] >= self.target_long or self.cross_t0== -1.0 or self.tick_close<-1000 or self.hourmin=='14:50')
					):
