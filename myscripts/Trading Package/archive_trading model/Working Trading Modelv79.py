...
class Strategy(bt.Strategy):
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			self.slope_of_slope_obv_t0 = round(self.inds.get(self.name_t0).get('slope_of_slope_obv')[0],3)
			...
			self.pday_open_t0 = round(self.inds.get(self.name_t0).get('prior_ohlc').lines.prior_open[0],3)
			self.pday_high_t0 =  round(self.inds.get(self.name_t0).get('prior_ohlc').lines.prior_high[0],3)
			self.pday_low_t0 =  round(self.inds.get(self.name_t0).get('prior_ohlc').lines.prior_low[0],3)
			self.pday_close_t0 =  round(self.inds.get(self.name_t0).get('prior_ohlc').lines.prior_close[0],3)
			self.pday_volume_t0 =  round(self.inds.get(self.name_t0).get('prior_ohlc').lines.prior_volume[0],3)
			"""
			...
			#print(d._name,self.dt,self.hourmin,d.volume[0],self.vwap_t0)
			self.vwap_t1 = round(self.inds.get(self.name_t1).get('vwap').lines.vwap[0],3)

			#Calculate same time bar, prior day
			self.intraday_volume_t0 = d.volume[-78]
			...
			self.stop_dist = self.stop_atr*self.p.atrdist
			...
			#VWAP STOP
			#self.long_stop = self.vwap_t0 * .995
			#self.short_stop = self.vwap_t0 * 1.005
			
			#Calculate Sizing
			if d.close[0] != 0:  #Protect against divide by 0 error
				self.maxsize = self.p.total_dollars_risked/d.close[0]
			else:
				self.maxsize = 0
			
			if self.stop_dist != 0:
				self.atrsize = self.p.dollars_risked_per_trade/self.stop_dist
			else:
				self.atrsize = 0
			...
			if (
				...
				if(	#Timeframe 0 signals
					...
					#and d.close[-1] < self.vwap_t0
					#and d.close[0]>self.rng_high
					and self.slope_obv_t0 > 0
					#and self.slope_of_slope_obv_t0 > 0
					#and self.slope_of_slope_obv_t0 > 0
					...
					#and self.pday_close_t0> self.support_t1
					and self.slope_t1 > 0
					#and self.sma1_t1 > self.sma2_t1
					#and self.adx_t1 > 20
					#and self.volume_t1 > self.intraday_volume_t1
				...
				#DEFINE SHORT ENTRY LOGIC
						...
						#and self.pday_close_t0< self.resistance_t1
						#and self.volume_t1 > self.intraday_volume_t1
						#and self.adx_t1 > 20
						#and self.adx_t1 < 40
						#and self.sma1_t1 < self.sma2_t1
...
def runstrat():
	...
	if not modelp.get('live_status'):
		...
		for i in range (0,len(results[0].datas),3):
			...
			cerebro.plot(end = plot_end, fmt_x_ticks = '%Y-%b-%d %H:%M', fmt_x_data = '%Y-%b-%d %H:%M',volume = True, style='candlestick',barup='olive', bardown='lightpink',volup = 'lightgreen',voldown='crimson')
