...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2017, 4, 3), #Dates for backtesting
	...
	def ind_params():
		params = dict(
			...
			ema1= 10,
			ema2= 20,
			hist= 70,  #For volume intraday over intraday period calculations
			atrperiod= 14,
			atrdist= 2,   
			slope_period=14,
			breakout_per=5, 
			...
			bollinger_dist=2,
			lookback=10,
			vwap_lookback=80,#Support/Resistance calcs
			)
		return params
...
class Strategy(bt.Strategy,UserInputs):
	...
	def __init__(self):
		...
		for i, d in enumerate(self.datas):
			...
			#First need to create data feed of average close prices/volume as numerator input into vwap calculation												
			if d._name == d._name[:-1]+'0' or d._name == d._name[:-1]+'1':
				self.inds[d._name]['cumvwap'] = btind.cumvwap(d,
	...
	def next(self):
		...
		dt = datetime.strftime(self.data.num2date(),'%H:%M:%S')
...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() >  self.max_ind_period():
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				#Calculate VWAP	
				if d._name == d._name[:-1]+'0' or d._name == d._name[:-1]+'1':
					self.vwap_t0 = round(self.inds.get(self.name_t0).get('cumvwap').lines.vwap[0],3)
					self.vwap_t1 = round(self.inds.get(self.name_t1).get('cumvwap').lines.vwap[0],3)
				...
				#5MIN timeframe
				if d._name == d._name[:-1]+'0' and len(self) > ((390/modelp.get('timeframe2'))*10): #ensure just for timeframe 1, and at least 65 periods load (1 every 6.5 hours for longest timeframe, or 10 times which is 65)
					self.volume_list_t0 = d.volume.get(ago=0,size=780)  #780 because their are 78 "5 min" periods each day, and we need 10 of them
				...
				#15MIN timeframe
				if d._name == d._name[:-1]+'1' and len(self) > ((390/modelp.get('timeframe2'))*10): #ensure just for timeframe 1, and at least 65 periods load (1 every 6.5 hours for longest timeframe, or 10 times which is 65)
					self.volume_list_t1 = d.volume.get(ago=0,size=260)  #260 because their are 26 "15 min" periods each day, and we need 10 of them
					self.period_volume_list_t1 = self.volume_list_t1[::-26]  #start at most recent value, and step back at 26 unit interval 
				...
				if (d._name == d._name[:-1]+'1'	#Trade basetimeframe only
					...
					if(	#self.slope_t1 > 0
						...
						d.close[0] > self.vwap_t0
						#self.cross_t1 > 0 #fast crosses above slow
						#and d.volume[0] > self.avg_volume_t1
						#and d.close[0] > self.vwap
						#self.slope_t1 > 0
						#and self.slope_t2 > 0
...
def runstrat():
	...
	if not modelp.get('live_status'):
		...
		for i in range (1,len(strats.datas),3):   #Assumes 3 timeframes used
			for j, d in enumerate(strats.datas):
				d.plotinfo.plot = i ==j	
