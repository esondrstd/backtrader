...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2017, 1, 3), #Dates for backtesting
...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() >  self.max_ind_period():
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				if (self.pos_dict[d._name][-1]!=0 and self.pos_dict[d._name][0] is 0 and self.inorder_dict[d._name][0] is True):
					self.inorder_dict[d._name][0] = False
						 
				print(d._name, dt, self.hourmin, cash_avail, pos_cost, self.pos_dict[d._name], self.inorder_dict.get(d._name)[0], self.close_t1, self.pbar_close_t1,)
				
				#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
				if (cash_avail > pos_cost
					and self.pos is 0
					and self.inorder_dict.get(d._name) is False
