...
class UserInputs():
	...
	def datalist(data_req):
		#datalist = ['SPY','XLU']
		datalist = ['SPY','IAU','TIP','AGG','XHB','DBA','VNQ','LQD','EWZ','XLU','MCD','XLK','VIX','TICK-NYSE',]
		#datalist = ['IAU','TIP','AGG','EMB','VNQ','XLU','SPY','XOM','LQD','EWZ','MCD','DBA','EWH','EWW','HYG','XLV','XRT','XLK','SHY','XHB']
		ibdatalist = ['SPY-STK-SMART-USD','AAPL-STK-SMART-USD','TICK-NYSE-I-NYSE','VIX-I-CBOE','TRIN-NYSE-I-NYSE']
		#ibdatalist = ['AUD.CAD-CASH-IDEALPRO','EUR.USD-CASH-IDEALPRO']
		if data_req == 'ib':
			return ibdatalist
		elif data_req == 'hist':
			return datalist
	...
	def model_params():
		params = dict(
			...
			writer='on', #export results to CSV output report 'on' or 'off'
...
#-------------------------------------------------------------------------------------------------------------------------	
			...
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				if not (d._name[:-1]=='VIX' or d._name[:-1]=='TICK-NYSE'):
					...
					if d._name == d._name[:-1]+'1': #For a single timeframe only (timeframe 1)
						...
						self.bullish_engulfing_pattern_t1= self.bullish_engulfing(d)
						self.bearish_engulfing_pattern_t1= self.bearish_engulfing(d)	
					...
					if d._name == d._name[:-1]+'2':	#For a single timeframe only (timeframe 2)
						self.bullish_engulfing_pattern_t2= self.bullish_engulfing(d)
						self.bearish_engulfing_pattern_t2= self.bearish_engulfing(d)	
	...
	def bullish_engulfing(self,mydata):
		#Candlestick pattern
		if (mydata.close[-1]< mydata.open[-1]
	...
	def bearish_engulfing(self,mydata):
		#Candlestick pattern
		if (mydata.close[-1]> mydata.open[-1]
			and mydata.open[0] > mydata.close[-1]
			and mydata.close[0] < mydata.open[-1]
	...
	def bullish_three_line_strike(self,mydata):
		#Candlestick pattern
		if (mydata.close[-3]< mydata.open[-3]
			and mydata.low[-2] < mydata.low[-3]
			and mydata.close[-2]< mydata.open[-2]
			and mydata.low[-1] < mydata.low[-2]
			and mydata.close[-1]< mydata.open[-1]
			and mydata.open[0]< mydata.close[-1]
			and mydata.close[0]> mydata.high[-3]
			and mydata.close[0] > mydata.open[0]
			):
			signal = True
		else:
	...
	def bearish_three_line_strike(self,mydata):
		#Candlestick pattern
		if (mydata.close[-3]> mydata.open[-3]
			and mydata.high[-2] > mydata.high[-3]
			and mydata.close[-2]> mydata.open[-2]
			and mydata.high[-1] > mydata.high[-2]
			and mydata.close[-1]> mydata.open[-1]
			and mydata.open[0]> mydata.close[-1]
			and mydata.close[0]< mydata.low[-3]
			and mydata.close[0] < mydata.open[0]
			):
			signal = True
		else:
...
def runstrat():
	...
	#Ensure stock lists have no duplicates
	...
	if UserInputs.model_params().get('writer')=='on':
		...
		csv_file = 'C:/Program Files (x86)/Python37-32/Lib/site-packages/backtrader/out/'
		csv_file += 'Strategy'
