...
class Strategy(bt.Strategy,UserInputs):
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			print(d._name, self.hourmin, self.cash_avail, self.pos, self.size_dict.get(d._name)[-1], self.inorder_dict[d._name], self.percK_t0, self.close_t0, self.target_long, self.target_short)
