...
class Strategy(bt.Strategy,UserInputs):
	...
	def __init__(self):
		...
		for x in range(0, len(self.datas), self.ticker_interval):
			...
			self.typprice_dict[d._name] = dict()
	
		for i, d in enumerate(self.datas):
			
			"""
			#d.datetime.datetime().strftime('%Y-%m-%d %H:%M:%S')
			self.hourmin = d.datetime.datetime().strftime('%H:%M')
			
			#Calculative cumulative # of periods within day for VWAP calc 	
	...
	def next(self):
		...
		#print(self.hourmin,self.dayperiod) 
...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() >  self.max_ind_period():
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				#Calculate VWAP - considering storing each value in dictionary to be referenced later	
				self.cumvol = math.fsum(d.volume.get(ago=0,size=self.dayperiod))
				self.typprice = round(((d.close[0]+d.high[0]+d.low[0])/3)*d.volume[0],2)
				
				#Should i use list or dictionary?
				self.typprice_dict[d._name]=self.typprice
				#self.cumtypprice = round(self.typprice.get(self.name_t0),2)
				
				#self.typprice_list.append(self.typprice
				#self.cumtypprice = sum(self.typprice_list[-self.dayperiod:])
				
				#if self.cumvol != 0:
					#self.vwap = self.cumtypprice/self.cumvol
					#print(self.vwap)
					
				print(d._name,self.hourmin, self.typprice_dict)
				#print(d._name,self.hourmin,self.dayperiod,d.volume.get(ago=0,size=5),d.close[0],d.high[0],d.low[0],self.cumvol,self.typprice)
						...
						if d._name == d._name[:-1]+'0':	#Trade basetimeframe only
							...
							if not (d._name[:-1]=='VIX' or d._name[:-1]=='TICK-NYSE' or d._name[:-1]=='TRIN-NYSE' or d._name[:-1]=='SPY'):
								if not modelp.get('live_status'):
									...
									print('VIX SMA is {} SPY SMA is {} TICK SMA is {} TRIN SMA is {}'.format(self.vixsma_t0,self.spysma_t0,self.ticksma_t0,self.trinsma_t0))  #This line proves i can reference single ticker indicator data outside of current data loop
