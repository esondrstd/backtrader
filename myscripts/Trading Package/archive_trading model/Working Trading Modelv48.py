...
class UserInputs():
	...
	def datalist(data_req):
		...
		#datalist = ['VIX','TICK-NYSE','SPY','XLU','IAU']
		datalist = ['SPY','XLU']
	...
	def model_params():
		params = dict(
			...
			start_date = date(2018, 7, 2), #Dates for backtesting
...
class Strategy(bt.Strategy,UserInputs):
	...
	def next(self):
		session_start = UserInputs.model_params().get('sessionstart')
		session_end = UserInputs.model_params().get('sessionend')
		start_date = UserInputs.model_params().get('start_date')
		end_date = UserInputs.model_params().get('start_date')
		
		mydate = "{} {}".format(start_date,session_start)
