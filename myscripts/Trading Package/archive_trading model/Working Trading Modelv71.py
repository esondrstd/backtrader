...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			trade_entry_time = '08:55',
			trade_exit_time = '10:30',
...
class Strategy(bt.Strategy,UserInputs):
	...
	def __init__(self):
		...
		for i, d in enumerate(self.datas):
			...
			#Trade Entry Logic (Long and Short)
			self.afford_pos=self.cash_avail > pos_cost
			self.no_pos = self.size_dict.get(d._name)[-1] == 0 and self.inorder_dict.get(d._name)[-1] == False
			self.data_loaded = self.prenext_done
			...
			if d._name == d._name[:-1]+'0': 
				self.myslope_t0 = self.inds[d._name]['slope']
				
				self.open_t0 = self.inds[d._name]['ohlc'].lines.open
				self.high_t0 = self.inds[d._name]['ohlc'].lines.high
				self.low_t0 = self.inds[d._name]['ohlc'].lines.low
				self.close_t0 = self.inds[d._name]['ohlc'].lines.close
				self.volume_t0 = self.inds[d._name]['ohlc'].lines.volume

				self.pbar_open_t0 =  self.inds[d._name]['prior_ohlc'].lines.prior_open
				self.pbar_high_t0 =  self.inds[d._name]['prior_ohlc'].lines.prior_high
				self.pbar_low_t0 =  self.inds[d._name]['prior_ohlc'].lines.prior_low
				self.pbar_close_t0 =  self.inds[d._name]['prior_ohlc'].lines.prior_close
				self.pbar_volume_t0 =  self.inds[d._name]['prior_ohlc'].lines.prior_volume
				
				self.vwap_t0 = self.inds[d._name]['vwap'].lines.vwap
				self.resistance_t0 = self.inds[d._name]['resistance']
				self.support_t0 = self.inds[d._name]['support']
				self.intraday_volume_t0 =  self.inds[d._name]['avgvolume'].lines.avgvolume_5min
				self.sma1_t0 =  self.inds[d._name]['sma1']
				self.sma2_t0 =  self.inds[d._name]['sma2']
				self.adx_t0 =  self.inds[d._name]['adx']
				#self.rsi_t0 =  self.inds[d._name]['rsi']
				self.percK_t0 = self.inds[d._name]['stochastic'].lines.percK
				#self.boll_top_t0 = self.inds[d._name]['bollinger'].lines.top
				#self.boll_mid_t0 = self.inds[d._name]['bollinger'].lines.mid
				#self.boll_bot_t0 = self.inds[d._name]['bollinger'].lines.bot
				self.highest_t0 =  self.inds[d._name]['highest']
				self.lowest_t0 =  self.inds[d._name]['lowest']
				self.avg_atr_t0 =  self.inds[d._name]['atr']
				...
				self.long_stop = d.close - self.stop_dist
				self.short_stop = d.close + self.stop_dist
				
				#Calculate Sizing
				self.maxsize = modelp.get('total_dollars_risked')/d.close
...
#-------------------------------------------------------------------------------------------------------------------------	
		...
		if self.maxtimeframe() >  self.max_ind_period():
			for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
				...
				dt = datetime.strftime(d.num2date(),'%Y-%m-%d')
				self.datadate=datetime.strftime(d.num2date(),'%H:%M:%S')
				self.hourmin = datetime.strftime(d.num2date(),'%H:%M')
				...
				print(self.myopen_t0[0],self.open_t0)
				print(self.enter_trade[0])
				...
				print(self.myslope_t0[0],self.slope_t0)
				...
				"""
				self.size = int(min(self.maxsize[0],self.atrsize[0]))  #Needs to be integer for IB to process
				self.cost = self.size * d.close  #Need intermidiary step to avoid method/float multiplication error
				#Ensure we have the cash to afford next position
				pos_cost = self.cost
				...
				print(d._name, dt, self.hourmin, self.cash_avail, self.pos, self.size_dict.get(d._name)[-1], self.inorder_dict[d._name], self.percK_t0[0], self.close_t0[0], self.target_long, self.target_short)
				
				#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
				if (self.hourmin >= modelp.get('trade_entry_time') and self.hourmin <= modelp.get('trade_exit_time')
					and self.afford_pos
					and self.no_pos
					...
					if(	#Timeframe 0 signals
						self.percK_t0[0] < 20
						#and self.slope_t0 < 0
						and self.volume_t0[0] > self.intraday_volume_t0[0]
						#and self.low_t0 < self.support_t0
						#and self.pbar_close_t0 < self.support_t0
						#Timeframe 1 signals
					...
					#DEFINE SHORT ENTRY LOGIC
							...
							self.percK_t0[0] > 80
							#and (self.high_t0 >= self.resistance_t0 and self.pbar_high_t0< self.resistance_t0)
							#and self.slope_t0 > 0
							and self.volume_t0[0] < self.intraday_volume_t0[0]
							#Timeframe 1 signals
				...
				#********************************EXIT LOGIC/ORDERS*********************************************
					...
					if (d._name == d._name[:-1]+'0' 
						...
						and (self.low_t0[0] <= self.target_short or self.hourmin=='14:45')  
						):		
					...
					#EXIT LOGIC FOR LONGS
						...
						and (self.high_t0[0] >= self.target_long or self.hourmin=='14:45')
						):
