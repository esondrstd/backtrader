...
class Strategy(bt.Strategy):
	...
	def notify_order(self, order):
		...
		if order.status == order.Completed:
			if order.isbuy() and order.executed.price:
				print('-'*80)
				...
				longstop_price = order.executed.price * (1.0 - .01)
				self.sell(exectype=bt.Order.Stop, price=longstop_price)
				print(f"Long Stop submitted at {longstop_price}")
					 
			elif order.issell() and order.executed.price:  # Sell
				print('-'*80)
				...
				shortstop_price = order.executed.price/(1.0 - .01)
				self.sell(exectype=bt.Order.Stop, price=shortstop_price)
				print(f"Short Stop submitted at {longstop_price}")
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			if d._name == 'TICK-NYSE0':
				self.tick_close = self.inds.get(self.name_t0).get('ohlc').lines.c[0]
			
			if d._name == 'SPY0':
				self.spy_close = self.inds.get(self.name_t0).get('ohlc').lines.c[0]
			...
			if self.hourmin=='08:30' and d._name!='TICK-NYSE0' and d._name==self.name_t0:
				#Determine open gap
				self.gap_dict[self.name_t0] = round(self.inds.get(self.name_t0).get('gap').lines.gap[0],3)

			if self.combosorted_dict is not None and self.name_t0 in self.combosorted_dict.keys():
				...
				self.target_short = self.target_short_dict.get(self.name_t0)
				self.target_long = self.target_long_dict.get(self.name_t0)
...
#********************************EXIT LOGIC/ORDERS*********************************************
				...
				if (d._name == self.name_t0 
					...
					self.exit_short_name = f'{self.name_t0} - EXIT SHORT!!'     
					self.exit_short = self.close(self.name_t0,
												name=self.exit_short_name)
					
					#print('{} {} EXIT SHORT {} - {} shares at {}'.format(self.dt, self.hourmin, self.name_t0,self.size_dict.get(self.name_t0)[-1],self.close_t0))
				...
				#EXIT LOGIC FOR LONGS
					...
					and (self.high_t0 >= self.target_long or self.tick_close < -1000 or self.hourmin=='14:45')
					and self.prenext_done  #Start trading after all prenext loads (live and backtest modes) 
					):
					
					#print('{} {} EXIT LONG {} - {} shares at {}'.format(self.dt, self.hourmin, self.name_t0,self.size_dict.get(self.name_t0)[-1],self.close_t0))
					#LONG EXIT ORDER - closes existing position and cancels outstanding stop-loss order
					self.exit_long_name = f'{self.name_t0} - EXIT LONG!!' 
					
					self.exit_long = self.close(self.name_t0,
												name=self.exit_long_name)
										
				
				#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
				if (d._name==self.name_t0
					and not self.name_t0[:-1]=='TICK-NYSE'
					and self.pos==0
					...
					if(	#Timeframe 0 signals
						...
						if not self.modelp.get('live_status'):
							...
							#Create Long Order
							long_name = f'{self.name_t0} - BUY BUY BUY'
							self.long_ord = self.buy(data=self.name_t0,
												size=self.size,
							...
							self.target_long_dict[self.name_t0] = self.target_long_price
							
							"""
							#Create Fixed Long Stop Loss
							long_stop_name = f'{self.name_t0} - STOPPED OUT OF LONG TRADE'  #Add info when notification output triggers
							self.long_stop_ord = self.sell(data=self.name_t0,
												size=self.size,
							...
							"""
							#print('{} {} BUY BUY BUY {} - {} shares at {}.  Stop price @ {}'.format(self.dt, self.hourmin,self.name_t0,self.size,d.close[0],self.long_stop))				
							...
							#Create Fixed Long Stop Loss
							long_stop_name =  f'{self.name_t0} - STOPPED OUT OF LONG TRADE'
							self.long_stop_ord = self.sell(data=self.name_t0,
												size=self.size,
												exectype=bt.Order.Stop,
												price=self.long_stop,
					...
					#DEFINE SHORT ENTRY LOGIC
							...
							if not self.modelp.get('live_status'):
								...
								short_name = f'{self.name_t0} - SELL, SELL, SELL'
								self.short_ord = self.sell(data=self.name_t0,
									 size=self.size,
									 exectype=bt.Order.Market,
									 transmit=True,
									 name=short_name,
								...
								self.target_short_dict[self.name_t0] = self.target_short_price
								"""									
								#Create Fixed Short Stop Loss	 
								short_stop_name =  f'{self.name_t0} - STOPPED OUT OF SHORT TRADE'
								self.short_stop_ord = self.buy(data=self.name_t0,
									size=self.size,
									exectype=bt.Order.Stop,
								...
								"""
								#print('{} {} SELL SELL SELL {} - {} shares at {}.  Stop price @ {}'.format(self.dt, self.hourmin, self.name_t0,self.size,d.close[0],self.short_stop))
