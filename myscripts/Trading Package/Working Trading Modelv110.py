...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','AAPL', 'ABBV', 'ABT', 'ACN', 'ADBE', 'ADI']
	...
	def model_params():
		params = dict(
			...
			timeframe1 = 30, #MINUTES
			timeframe2 = 240, #MINUTES
			timeframe1on = True,
			timeframe2on = True,
...
class Strategy(bt.Strategy):
	...
	params = dict(
			...
			bollinger_dist=2,
			lookback=10,
			rank = 6, #How many tickers to select from ticker list
			)
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			self.pos = self.getposition(d).size  #Get position size for entry/exit logic
			...
			if self.hourmin=='14:50' and self.pos and d._name == d._name[:-1]+'0' :
				self.eod_exit(d,d._name)
			
			if self.pos and self.tick_close > 1000:
				self.exit_trade(d,d._name,'long')
			
			if self.pos and self.tick_close < -1000:
				self.exit_trade(d,d._name,'short')
			
			#DEFINE OVERALL ENTRY LOGIC FOR LONG AND SHORT
			if self.entry_rules(d,d._name) and self.pos==0:  #Check that entry rules are met
				
				#Check if long and short signals are true - if so, submit trade
				#if self.signal_test(d,d._name):
				if self.signal_morn_break(d,d._name,'long'):
					self.buyorder(d,d._name,self.trade_size(d._name))

				if self.signal_morn_break(d,d._name,'short'):
					self.sellorder(d,d._name,self.trade_size(d._name))
					

	#*********************************************************************************************************************

	def exit_trade(self,d,dname,direction):
		...
		if(	direction == 'short'
			...
			print(f'{self.dt} {self.hourmin} EXIT SHORT {dname} - {self.pos} shares at {d.close[0]}')
			...
			print(f'{self.dt} {self.hourmin} EXIT LONG {dname} - {self.pos} shares at {d.close[0]}')
			self.exit_long_name = f'{dname} - Exit Long Trade'
			
			self.exit_long = self.close(dname,
										name=self.exit_long_name)
			
	
	def eod_exit(self,d,dname):
	...
	def entry_rules(self,d,dname):
		#!!!!!!!!!!MAY NEED TO CREAT POSITION DICTIONARY FOR LIVE TRADING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	def sellorder(self,d,dname,mysize):
		...
		if dname==d._name[:-1]+'0':
			...
			if mysize:
				self.target_short = round(d.open [0] - ((self.p.dollars_risked_per_trade*self.p.target)/mysize),2)
	...
	def buyorder(self,d,dname,mysize):
		...
		if dname==dname[:-1]+'0':
			...
			#Create Target Price
			if mysize:
				self.target_long = round(d.open[0] +((self.p.dollars_risked_per_trade*self.p.target)/mysize),2)
			...
			elif self.modelp.get('live_status'):  #Exclue name from Live trading since IB can't process
				if self.data_live:
					self.long_ord = self.buy(data=dname,
										size=mysize,
										exectype=bt.Order.Market,
	...
	def signal_test(self,d,dname):
		if d.close[0] > d.close[-1] and dname==d._name[:-1]+'0':
			return True
		else:
			return False
	...
	def regime_early_bullish(self,d,dname):
		...
		"""
		if dname==dname[:-1]+'1':
			self.slope_obv_t1 = self.inds.get(dname).get('slope_obv')[0] #Get OBV slope
			self.slope_t1 = self.inds.get(dname).get('slope')[0]  #Calc slope for time1
			self.slope_of_slope_t1 = self.inds.get(dname).get('slope_of_slope')[0]  #Calc slope for time1
			self.ema1_t1 = self.inds.get(dname).get('ema1')[0]
			self.ema2_t1 = self.inds.get(dname).get('ema2')[0]
			self.slope_ema1_t1 = self.inds.get(dname).get('slope_ema1')[0]
			self.slope_ema2_t1 = self.inds.get(dname).get('slope_ema1')[0]
			self.adx_t1 = self.inds.get(dname).get('adx')[0]
			self.slope_adx_t1 = self.inds.get(dname).get('slope_adx')[0]
			self.slope_ema_width_t1 = self.inds.get(dname).get('slope_ema_width')[0]
			self.boll_mid_t1 = self.inds.get(dname).get('bollinger').lines.mid[0]
			
		
		if dname==dname[:-1]+'2':
			self.slope_obv_t2 = self.inds.get(dname).get('slope_obv')[0] #Get OBV slope
			self.slope_t2 = self.inds.get(dname).get('slope')[0]  #Calc slope for time2
			self.slope_of_slope_t2 = self.inds.get(dname).get('slope_of_slope')[0]  #Calc slope for time1
			self.ema1_t2 = self.inds.get(dname).get('ema1')[0]
			self.ema2_t2 = self.inds.get(dname).get('ema2')[0]
			self.slope_ema1_t2 = self.inds.get(dname).get('slope_ema2')[0]
			self.slope_ema2_t2 = self.inds.get(dname).get('slope_ema2')[0]
			self.adx_t2 = self.inds.get(dname).get('adx')[0]
			self.slope_adx_t2 = self.inds.get(dname).get('slope_adx')[0]
			self.slope_ema_width_t2 = self.inds.get(dname).get('slope_ema_width')[0]
			self.boll_mid_t2 = self.inds.get(dname).get('bollinger').lines.mid[0]


		#Define signal criteria
		if (self.adx_t1 > 20
			and self.adx_t2 > 20
			and self.slope_adx_t1 > 0
			and self.slope_adx_t2 > 0
			and self.ema1_t1 > self.ema2_t1
			and self.ema1_t2 > self.ema2_t2
			and self.slope_ema1_t1 > 0
			and self.slope_ema2_t1 > 0
			and self.slope_ema_width_t1 > 0
			and self.slope_ema_width_t2 > 0
			and self.slope_t1 > 0
			and self.slope_t2 > 0
			and self.slope_of_slope_t1 > 0
			and self.slope_of_slope_t2 > 0
			and self.slope_obv_t1 > 0
			and self.slope_obv_t2 > 0
			and d.close[0] > self.boll_mid_t1
			and d.close[0] > self.boll_mid_t2
			):
	...
	def regime_late_bullish(self,d,dname):
		...
		if dname==dname[:-1]+'1':
			...
			self.slope_adx_t1 = self.inds.get(dname).get('slope_adx')[0]
			self.slope_ema_width_t1 = self.inds.get(dname).get('slope_ema_width')[0]
			self.rsi_t1 = round(self.inds.get(dname).get('rsi')[0],2)
		...
		if dname==dname[:-1]+'2':
			...
			self.slope_adx_t2 = self.inds.get(dname).get('slope_adx')[0]
			self.slope_ema_width_t2 = self.inds.get(dname).get('slope_ema_width')[0]
			self.rsi_t2 = round(self.inds.get(dname).get('rsi')[0],2)
	...
	def regime_neutral(self,d,dname):
		#Define Variables
		"""
		if dname==dname[:-1]+'0':
			self.vwap_t0 = self.inds.get(dname).get('vwap').lines.vwap[0]
		...
		if dname==dname[:-1]+'1':
			self.adx_t1 = self.inds.get(dname).get('adx')[0]
			self.rsi_t1 = round(self.inds.get(dname).get('rsi')[0],2)
			self.resistance_t1 = self.inds.get(dname).get('resistance')[0]
			self.support_t1 = self.inds.get(dname).get('support')[0]
		
		if dname==dname[:-1]+'2':
			self.adx_t2 = self.inds.get(dname).get('adx')[0]
			self.rsi_t2 = round(self.inds.get(dname).get('rsi')[0],2)
			self.resistance_t2 = self.inds.get(dname).get('resistance')[0]
			self.support_t2 = self.inds.get(dname).get('support')[0]
		...
		if (self.adx_t1 < 20
			...
			and self.rsi_t2 <= 60
			and self.rsi_t2 >= 40
			and d.close[0] < self.resistance_t1
			and d.close[0] > self.support_t1
			and d.close[0] < self.resistance_t2
			and d.close[0] > self.support_t2
			):
	...
	def regime_late_bearish(self,d):
...
#RUN STRATEGY
	...
	#PLOT RESULTS 	
	#myplot(cerebro,results[0])	#plot results

#************************************************************************************************************************************			
def myplot(cerebro,results):
	...
	#only get data for timeframe 0
	for i,d in enumerate(results.datas):
