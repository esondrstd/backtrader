...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','AAPL']
		...
		ibdatalist = []  #'AAPL-STK-SMART-USD'
		ibforex_datalist = ['EUR','GBP','AUD']  #'EUR','GBP','AUD'
...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.short_ord = defaultdict(list)
		self.short_stop_ord = defaultdict(list)
		self.long_ord = defaultdict(list)
		self.long_stop_ord = defaultdict(list)
...
#**************************************************************************************************************************************
	...
	def notify_order(self, order):
		...
		if order.status == order.Completed:
			if order.isbuy():
				print('-'*80)
				print(f"BUY EXECUTED, Date: {self.dt} {self.hourmin} Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value),2}, Size {order.executed.size}")
			
			elif order.issell():
				print('-'*80)
				print(f"SELL EXECUTED, Date: {self.dt} {self.hourmin} Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value,2)}, Size {order.executed.size}")
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterrate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			#Submit Short Order
			print(d._name,len(d))
			print(d._name,self.dt,self.hourmin)
			if d._name==d._name[:-1]+'0' and self.data_live: #and self.data_live:
			
			
				self.sellorder(d,self.trade_size(d))
				"""
				#Calculate STOP (ATR BASED)
				self.short_stop = self.inds.get(d._name).get('atr_stop').lines.short_stop[0]
				
				self.short_ord = self.sell(data=d,
										 size=self.trade_size(d),
										 exectype=bt.Order.Market,
										 transmit = False)
										
				#Create Fixed Short Stop Loss	 
				self.short_stop_ord = self.buy(data=d,
									size=self.trade_size(d),
				...
				print(f'{self.dt} {self.hourmin} SELL SELL SELL {d._name} - {self.trade_size(d)} shares at {d.close[0]}.  Stop price @ {self.short_stop} Pos {self.pos} Cash {self.broker.getcash()}')
				"""
				"""
				#Create Long Order
				#FOR BRACKET TRADING
				self.long_ord = self.buy_bracket(data=d,size=100,limitprice=1000,stopprice=2)
				"""
			
			
			"""
			#Identify market regime
			#self.regime_early_bullish(d)
			...
			if self.hourmin=='14:50' and self.pos and d._name == d._name[:-1]+'0' :  #can't exit at 14:55 because cerebro submits at order at open of next bar
				self.eod_exit(d)
			...
			if self.pos==0 and self.entry_rules(d):  #Check that entry rules are met
				...
				if self.signal_test_long(d):
					...
					#Create Target Price
					if self.trade_size(d):
						self.target_long = round(d.open[0] +((self.p.dollars_risked_per_trade*self.p.target)/self.trade_size(d)),2)
					...
					#Track stops to ensure they can be cancelled later on market exit
					self.long_stop_dict[d._name].append(self.long_stop_ord)
					print(f'{self.dt} {self.hourmin} BUY BUY BUY {d._name} - {self.trade_size(d)} shares at {d.close[0]}.  Stop price @ {self.long_stop}')
				...
				if self.signal_test_short(d):
					...
					if self.trade_size(d):
						self.target_short = round(d.open [0] - ((self.p.dollars_risked_per_trade*self.p.target)/self.trade_size(d)),2)
					...
					self.short_stop_dict[d._name].append(self.short_stop_ord)
					print(f'{self.dt} {self.hourmin} SELL SELL SELL {d._name} - {self.trade_size(d)} shares at {d.close[0]}.  Stop price @ {self.short_stop}')
			
			"""

	#*********************************************************************************************************************
	def sellorder(self,d,mysize):
		...
		if d._name==d._name[:-1]+'0':
			...
			print(f'{self.dt} {self.hourmin} SELL SELL SELL {d._name} - {mysize} shares at {d.close[0]}.  Stop price @ {self.short_stop}')


	def buyorder(self,d,mysize):
		...
		if d._name==d._name[:-1]+'0':
			...
			elif self.modelp.get('live_status') and self.data_live:  #Exclue name from Live trading since IB can't process
				self.long_ord[d._name].append(self.buy(data=d._name,
									size=mysize,
									exectype=bt.Order.Market,
									transmit=True))
				
				#Create Fixed Long Stop Loss
				self.long_stop_ord[d._name] = self.sell(data=d._name,
									size=mysize,
									exectype=bt.Order.Stop,
									price=self.long_stop,
									transmit=True,
									parent=self.long_ord.get(d._name))			
				
				#Track if currently in an order or not
				self.long_stop_dict[d._name].append(self.long_stop_ord.get(d._name))
				
			print(f'{self.dt} {self.hourmin} BUY BUY BUY {d._name} - {mysize} shares at {d.close[0]}.  Stop price @ {self.long_stop}')
	...
	def exit_trade(self,d,direction):
		...
		if(	direction == 'short'
			...
			print(f'{self.dt} {self.hourmin} EXIT LONG {d._name} - {self.pos} shares at {d.close[0]}')
	...
	def rel_volume(self,d):
		...
		if d._name==d._name[:-1]+'0' and len(self) > (self.data_feed_count * 78 * 5):  #need enough data to look back 5 days
			self.rel_volume_t0 = self.inds.get(d._name[:-1]+'0').get('rel_volume').get(ago=78,size=1)
	...
	def signal_morn_break(self,d,direction):
		#Calc timeframe 0 
		self.slope_obv_t0 = self.inds.get(d._name).get('slope_obv')[0] #Get OBV
		...
		if direction =='long':
			if(	
				d._name == d._name[:-1]+'0'
				#and d._name in self.topsorted_dict.keys()
