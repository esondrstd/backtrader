...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		"""
		#-------------------STOCK SELECTION BASED ON DEFINED CRITERIA-----------------------------------------
		if self.hourmin=='09:00':
		...
		if self.hourmin>='09:05':
			for i in self.merged_dict.keys():
				mydata = self.datas_dic.get(i)
				d=mydata
				...
				if self.pos==0 and self.entry_rules(d) and d._name!='TICK-NYSE0':  #Check that entry rules are met: 		
	...
	def rank_perc(self):
		...
		self.sortflag = 1
		print(f'{self.dt} {self.hourmin} Top Sort: {self.rtop_dict}, Bottom Sort: {self.rbot_dict}')
