...
#**************************************************************************************************************************************
	...
	def pairs_entry_exit(self,z_entry_threshold=2.0, z_exit_threshold=1):
		...
		for i in self.adfpval:
			...
			if self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and (exit_signal or zscore_now >= self.stop_dict.get(name)[-1] or self.hourmin=='14:45'):
				...
				if zscore_now >= self.stop_dict.get(name)[-1]:
					print(f'STOPPED OUT')
			...
			if total_entry_signals !=0:
				...
				if total_entry_signals and not self.inorder_dict.get(f'{t1}/{t2} inorder')[-1] and self.hourmin<'11:00' and cash_avail > self.p.total_dollars_risked:
					if weights_t1>0:
