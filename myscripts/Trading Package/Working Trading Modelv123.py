...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.hourmin=='08:30' and not self.df_closes.empty:
	...
	def calc_spread_zscore(self,df):	
		for i in self.adfpval:
			...
			#For position sizing - for every unit bought of stock 1, buy hratio units of stock 2
			t1weight = 1
			t2weight = 1*hratio
	...
	def pair_regression(self,df):
		...
		print("NUMBER OF PASSING ADF PAIRS",len(self.adfpval),self.adfpval)		
	...
	def update_df(self,d):
		if not self.df_closes.empty and self.newdf.empty:
			...
			self.calc_spread_zscore(self.newdf) #update spreads and zscore columns to dataframe for every 'next' iteration
			self.pairs_signal(self.newdf)  #update long, short, and exit signal columns to dataframe for every next iteration
		
		print(self.newdf)
		print(self.newdf['longs'])
		pritn(self.newdf[f'zscore {t1}/{t2}'])
