...
class Strategy(bt.Strategy):
	...
	params = dict(
			...
			boll_dist = 2,
			lookback = 10,
			rank = 4, #How many tickers to select from ticker list
			)
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		if self.close_dict is not None:
			#finaldf = self.df.join(self.newdf,how='outer')
			mycorr = self.newdf.corr()
			print(mycorr)
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if (self.cor_counter  < self.p.correl*3) and d._name==d._name[:-1]+'0':
				self.cor_counter += 1
				
			else:
				if d._name==d._name[:-1]+'0' and self.hourmin == '08:30' and not d._name[:-1]=='TICK-NYSE':
					if d._name==self.data0._name:  #only need date series once
						self.date_dict[d._name] = [datetime.strftime(d.num2date(x),'%d-%m-%Y %H:%M') for x in d.datetime.get(size=self.p.correl)]
						self.df = pd.DataFrame.from_dict(self.date_dict)
						self.df.rename(columns={d._name:'date'}, inplace=True)
						#self.df.set_index('date', inplace=True)
						
						self.df = self.df.iloc[1:]  #remove 1st row of dataframe so records match to added columns
						print(self.df)
						
					# stack the closes together into a numpy array for ease of calculation
					#self.closes = np.stack(([d.close.get(0,self.p.correl).tolist()]))
					self.closes= d.close.get(0,self.p.correl)
					rets = np.diff(np.log(self.closes)).round(4)
					self.close_dict[d._name] = [x for x in rets]
					
					#self.myclose_dict[d._name]['close'].append([x for x in rets])
					#self.myclose_dict[d._name]['date'] = [datetime.strftime(d.num2date(x),'%d-%m-%Y %H:%M') for x in d.datetime.get(size=self.p.correl)]
					
					#self.close_dict[d._name]['close'] = np.stack(([d.close.get(0,self.p.correl)]))
					#self.close_dict[d._name]['date'].append([datetime.strftime(d.num2date(x),'%d-%m-%Y %H:%M') for x in d.datetime.get(size=self.p.correl)])
					#df = pd.DataFrame.from_dict(self.close_dict, orient='index')
					#print(df)
					#self.closes = d.close.get(0,self.p.correl)
					#print(self.closes)
					#self.close_dict[d._name].append(self.closes)
					#print(self.close_dict)
					#print(self.date_dict)
					#df = pd.Series(self.date_dict).to_frame('date')
					...
					if self.main_df.empty:
						...
						self.main_df = self.main_df.join(newdf, how='outer')
					"""	
					#df = df.join(pd.Series(self.close_dict[d._name]),how='outer')
					#df[d._name] = self.close_dict.get(d._name)
					#df = pd.DataFrame.from_dict(self.myclose_dict)
					#print(df)
					#print(f'****************************************LENGTH OF INDEX IS {df.count()}, LENGTH OF DICT IS {len(self.close_dict.get(d._name))}')
					
					#newdf = df.join(pd.Series(self.close_dict[d._name]).to_frame(d._name),how='outer')
					#df.set_index('date', inplace=True)
					
					#df.explode('date')
					#df = pd.Series(self.date_dict).to_frame('date').join(pd.Series(self.close_dict).to_frame(d._name), how='outer')
					
					#df = pd.DataFrame(list(self.close_dict.items()),columns = ['ticker','returns'])
					"""
					if self.main_df.empty:
						self.main_df = df
					else:
						self.main_df = self.main_df.join(df, how='outer')
					"""

					"""
					for k, v in self.close_dict.items():
						mycorr = np.corrcoef(self.close_dict.get('SPY0'), v)
				...
				#print(mycorr)
				#df.corr(method='pearson')
				#self.correl_dict = pd.DataFrame.to_dict(df, orient='index')
