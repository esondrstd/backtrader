...
#**************************************************************************************************************************************
	...
	def create_pairs(self,d,signif):
		...
		for i, (ticker1, ticker2) in enumerate(all_pairs): 	
			...
			t1_data = np.array(self.justclose_dict.get(ticker1))
			t2_data = np.array(self.justclose_dict.get(ticker2))
			combined_data = np.vstack((t1_data, t2_data)).T
			
			# The second and third parameters indicate constant term, with a lag of 1. 
			result = coint_johansen(combined_data, 0, 1)  #testing only 2 tickers at a time.  Can not conduct test with more than 12 tickers
	...
	def pair_regression(self,d):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			(m, c, rvalue, pvalue, stderr) = stats.mstats.linregress(beta, Y)
			self.hratio = m
	...
	def plot_spread(self):
		if self.adfpval:
			fig = plt.figure()
			cols = 2
			rows = int(len(self.pair_spread_dict)/cols)

			for i, pair in enumerate(self.pair_spread_dict):
				if i <= (rows*cols)-1:
					t1_name = pair.get('t1')
					t2_name = pair.get('t2')
					...
					ax.set_ylabel(f'{t1_name} data', color='g')
					ax.set_title(f'{t1_name}/{t2_name} Spreads')
