...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if  d._name==d._name[:-1]+'1' and d._name!='TICK-NYSE1':
				...
				#self.justdate_dict[d._name] = d.datetime.get(0,self.p.pairs_lookback)
	...
	def pairs_exit(self,z_exit_threshold=0):
		...
		for i in self.adfpval:
			...
			spread_now = round(hratio_t1 * t1_data[0] + hratio_t2 * t2_data[0],4) #create array of hedge ratio * close price
			np.append(self.pair_spread_dict[f'spread {t1}/{t2}'],spread_now)
			zscore_now = stats.zscore(self.pair_spread_dict.get(f'spread {t1}/{t2}')[0][-self.p.pairs_lookback:])
			np.append(self.pair_zscore_dict[f'zscore {t1}/{t2}'][0],zscore_now[-1])
