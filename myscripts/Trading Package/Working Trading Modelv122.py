...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','AAPL','INTC','ACN','ADBE', 'AMZN', 'ANTM', 'ADI', 'ADP', 'ADSK', 'AEP', 'AFL', 'AGG', 'AGN', 'ALGN', 'ALL', 'ALXN']
...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		#Get correlations, rank them, and store correlations pairs and value in dictionary
		if self.hourmin=='08:35' and not self.df_closes.empty:
			#self.rank_correl(self.data,self.df_returns)
			#self.myplot(self.data,self.df_closes)
			self.create_pairs(self.df_closes,self.p.signif)
			self.regression(self.df_closes)
			...
			self.plotdf(self.df_closes,self.adfpval)
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			#if self.hourmin=='08:35' and d._name==d._name[:-1]+'1' and d._name!='TICK-NYSE1' and not self.df_returns.empty:
				#self.plotratio(d,self.df_closes)
				#self.adfuller_test(d,self.df_closes)
	...
	def regression(self,df):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			stock1_open = df[stock1].iloc[-1]
			
			stock2 = pair.get('t2')
			stock2_hratio = pair.get('hratio_t2')
			stock2_open = df[stock2].iloc[-1]
			
			print(f'Stock 1: {stock1} of pair {i}: {stock1}/{stock2} Hedge ratio: {stock1_hratio} Open Price: {stock1_open}')
			print(f'Stock 2: {stock2} of pair {i}: {stock1}/{stock2} Hedge ratio: {stock2_hratio} Open Price: {stock2_open}')
			...
			self.adfuller_test(results,stock1,stock2,self.p.signif)
						
			# Create the spread and then a z-score of the spread
			#print ("Creating the spread/zscore columns...")
			#self.pair_spread[f'{stock1}{stock2}'] = stock1_open - stock2_hratio*stock2_open
			#self.pair_zscore[f'pair{i}'] = self.pair_spread[f'{stock1}{stock2}']  - np.mean(self.pair_spread[f'{stock1}{stock2}'] )/np.std(self.pair_spread[f'{stock1}{stock2}'] )
			#print(f"PAIR SPREAD = {self.pair_spread[f'{stock1}{stock2}']}")
	...
	def create_pairs(self,df,signif):
		...
		for i, (ticker1, ticker2) in enumerate(all_pairs): 	
			...
			pair_closes = df[[ticker1, ticker2]].dropna()
			...
			if np.all(result.lr1 >= trace_crit_value) and np.all(result.lr2 >= eigen_crit_value):
				...
				print(f'Pair:{ticker1} {ticker2}, Ticker: {ticker1} - Trace Statistic {result.lr1[0]} is > Critical Value {trace_crit_value[0]} and Max Eigen Val Statistic {result.lr2[0]} > {eigen_crit_value[0]} Critical Value, Hedge Ratio: {hedge_ratio[0]}')
				print(f'Pair:{ticker1} {ticker2}, Ticker: {ticker2} - Trace Statistic {result.lr1[1]} is > Critical Value {trace_crit_value[1]} and Max Eigen Val Statistic {result.lr2[1]} > {eigen_crit_value[1]} Critical Value, Hedge Ratio: {hedge_ratio[1]}')
	...
	def create_df(self,d):
		#Create dataframe of returns that can be used for correlation calculations
	...
	def adfuller_test(self,ols_results,t1_name,t2_name,signif):
		...
		if p_value <= signif:
			...
			self.adfpval.append(dict(t1=t1_name,t2=t2_name,pval=p_value))
		...
		"""
		# If second differencing needed to pass ADF test
