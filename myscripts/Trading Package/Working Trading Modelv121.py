...
class UserInputs():
	...
	def datalist(data_req):
		...
		datalist = ['SPY','XLU','TICK-NYSE','XHB','AAPL','INTC','ACN','ADBE', 'AMZN', 'ANTM']
...
#**************************************************************************************************************************************
	...
	def pairs(self,df):
		...
		for i, (ticker1, ticker2) in enumerate(all_pairs): 	
			...
			hedge_ratio = list(result.evec[0])	#The first column of eigenvectors contains the best weights
			...
			confidence_level_col = confidence_level_cols[90]
			...
			if np.all(result.lr1 >= trace_crit_value) and np.all(result.lr2 >= eigen_crit_value):
				...
				print(f'Pair:{ticker1} {ticker2} - Trace Statistic {result.lr1} is > Critical Value {trace_crit_value} and Max Eigen Val Statistic {result.lr2} > {eigen_crit_value} Critical Value')
				print(f'Pair:{ticker1} {ticker2} - Hedge Ratio: {hedge_ratio}')
				
				self.cointegrating_pairs.append(dict(t1=ticker1,t2=ticker2,hratio_t1=hedge_ratio[0],hratio_t2=hedge_ratio[1]))
				#self.plotdf(ticker1,ticker2,df[ticker1],df[ticker2])
		...
		#Extract pair names, hedge ratios, and open price for day
	...
	def coint(self,d,df):
		...
		out = coint_johansen(df,det_order=-1, k_ar_diff=1)  #det_order of -1 = no deterministic terms, 0 is constant, 1 is linear term
		alpha=0.01
	...
	def plotratio(self,t1_name,t2_name,ticker1,ticker2):
			ratio = ticker1/ticker2
			plot1 = ratio.plot.line(title=f'Price Ratio between {t1_name} and {t2_name}')
			plt.axhline(ratio.mean(), color='red', linestyle='--')
			plot1.set_ylabel('Price Ratio')
			plt.show()
	

	def myplot(self,d,df):
		# Plot
		fig, axes = plt.subplots(nrows = int(len(df.columns)/2) , ncols=2, dpi=120, figsize=(10,6))
	...
	def plotdf(self,t1_name,t2_name,ticker1,ticker2):
	...
	def adfuller_test(self,d,df,signif=0.05):
		...
		# Print Summary
		print(f'    Augmented Dickey-Fuller Test on "{d._name}"', "\n   ", '-'*47)
