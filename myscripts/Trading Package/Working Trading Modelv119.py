...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if self.hourmin=='08:30' and d._name!='TICK-NYSE0' and d._name==d._name[:-1]+'0':
				...
				self.closes = self.inds.get(d._name[:-1]+'0').get('close').get(size=self.p.correl)
	...
	def rank_correl(self):
		...
		#print(mycorr)
		spy_ranked = mycorr["SPY0"].sort_values(ascending=False).dropna() #get just SPY column in dataframe, then sort
		print(f'Top Positive Correlations to SPY: {spy_ranked.nlargest(3)} Top Negative: {spy_ranked.nsmallest(3)}')
		print(f'Return just ticker names of top SPY correlations: {spy_ranked.nlargest(3).index}')	#returns ticker pair list of highest ranked correlations
		"""
