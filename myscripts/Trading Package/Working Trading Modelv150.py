...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			if d._name==d._name[:-1]+'0' and (d._name in self.rtop_dict.keys() or d._name in self.rbot_dict.keys()) and self.pos==0 and self.entry_rules(d) and d._name!='TICK-NYSE0':  #Check that entry rules are met: 		
	...
	def pull_back(self,d):
		...
		#print(f'{self.dt}, {d._name}, Close:{d.close[0]}, TICK:{self.tick_close[0]}, OBV Slope:{self.slope_obv_t0}, VWAP: {self.vwap_t0}, Slope T1: {self.slope_t1}, Stoch: {self.percK_t0}')
