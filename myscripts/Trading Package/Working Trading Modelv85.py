...
class Strategy(bt.Strategy):
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterate over all datas so atr and sizing can be adjusted for multiple time frame user parameters
			...
			self.pday_open_t0 = round(self.inds.get(self.name_t0).get('priorday').lines.prior_open[0],3)
			self.pday_high_t0 =  round(self.inds.get(self.name_t0).get('priorday').lines.prior_high[0],3)
			self.pday_low_t0 =  round(self.inds.get(self.name_t0).get('priorday').lines.prior_low[0],3)
			self.pday_close_t0 =  round(self.inds.get(self.name_t0).get('priorday').lines.prior_close[0],3)
			self.pday_volume_t0 =  round(self.inds.get(self.name_t0).get('priorday').lines.prior_volume[0],3)
			...
			print(d._name, self.dt, self.hourmin, self.long_stop,self.short_stop,self.size,self.cost)
