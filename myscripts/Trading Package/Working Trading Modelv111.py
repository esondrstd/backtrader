...
#**************************************************************************************************************************************
	...
	def next(self):
		...
		for i, d in enumerate(self.datas):  #Need to iterrate over all datas so atr and sizing can be adjusted for multiple time frame user parameters	
			...
			#print("outside of entry rules")
			if self.pos==0 and self.entry_rules(d):  #Check that entry rules are met
				#print("enter into entry rules")
				#Check if long and short signals are true - if so, submit trade
				#if self.signal_test_long(d):
				#if self.signal_morn_break(d,'long'):
					#print("inside signal test, ready to submit buy order")
					#self.buyorder(d,self.trade_size(d))

				#if self.signal_morn_break(d,'short'):
				if self.signal_test_short(d) and self.data_live:
					#print("inside signal test, ready to submit sell order")
					...
					#self.short_stop_dict[d._name].append(self.short_stop_ord.get(d._name))
	...
	def entry_rules(self,d):
		...
		if (not d._name[:-1]=='TICK-NYSE'
			and (self.hourmin>='09:00'and self.hourmin<='15:00')
	...
	def sellorder(self,d,mysize):
		...
		if d._name==d._name[:-1]+'0':
			...
			if not self.modelp.get('live_status'):
				...
				print("We in sell land")
				self.short_ord[d._name].append(self.sell(data=d._name,
										 size=mysize,
										 exectype=bt.Order.Market,
										 transmit = False))
										
				#Create Fixed Short Stop Loss	 
				self.short_stop_ord[d._name] = self.buy(data=d._name,
									size=mysize,
									exectype=bt.Order.Stop,
									price = self.short_stop,
									transmit=True,
									parent=self.short_ord.get(d._name))
