...
#**************************************************************************************************************************************
	...
	def pair_regression(self,d):
		...
		for i, pair in enumerate(self.cointegrating_pairs):
			...
			beta_set = np.array(self.justclose_dict.get(stock1))
			beta = np.vstack([beta_set, np.ones(len(beta_set))]).T
			Y_set = np.array(self.justclose_dict.get(stock2))
			Y = Y_set.reshape((len(Y_set), 1))

			results = np.linalg.lstsq(beta, Y, rcond=None)[0]
			#results = stats.linregress(beta,Y)
			m, c = np.linalg.lstsq(beta, Y, rcond=None)[0]	
			#m, c = stats.linregress(beta,Y)[0]		
			self.hratio = m
			...
			for n, value in enumerate(Y):
				...
				print(f'{stock1}/{stock2} {self.errors}') 
