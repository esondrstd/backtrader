...
from backtrader.feeds import mysql2
...
class UserInputs():
	...
	def model_params():
		params = dict(
			...
			start_date = date(2019,9,1), #Dates for backtesting
...
#**************************************************************************************************************************************
	...
	def notify_order(self, order):
		if order.status == order.Completed:
			if order.isbuy() and self.pos==0:
				print(f"{order.data._name} ENTER LONG POSITION, Date: {self.dt} {self.hour} {self.minute} Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value,2)}, Size {order.executed.size}, Type {order.getordername()}")
			
			if order.isbuy() and self.pos < 0:
				print(f"{order.data._name} EXIT SHORT POSITION, Date: {self.dt} {self.hour} {self.minute}  Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value,2)}, Size {order.executed.size}, Type {order.getordername()}")
			
			if order.issell() and self.pos==0:
				print(f"{order.data._name} ENTER SHORT POSITION, Date: {self.dt} {self.hour} {self.minute}  Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value,2)}, Size {order.executed.size}, Type {order.getordername()}")
			
			if order.issell() and self.pos > 0:
				print(f"{order.data._name} EXIT LONG POSITION, Date: {self.dt} {self.hour} {self.minute}  Price: {round(order.executed.price,2)}, Cost: {round(order.executed.value,2)}, Size {order.executed.size}, Type {order.getordername()}")
	...
	def notify_trade(self, trade):
		if trade.isclosed:
			print(f"{trade.data._name} POSITION CLOSED {self.dt} {self.hour} {self.minute} Price: {round(trade.price,2)}, Net Profit: {round(trade.pnl,2)}")
...
def data_backtest(cerebro,start_date,end_date,session_start,session_end):
	...
	for i,j in enumerate(datalist):
		...
		data = mysql2.MySQLData(dbHost = host,
								dbUser = user,
								dbPWD = password,
								dbName = database,
								table = table,
