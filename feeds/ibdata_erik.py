...
class IBData(with_metaclass(MetaIBData, DataBase)):
    ...
    def parsecontract(self, dataname):
        ...
        """
        #ERIK ADDED CODE to get around parsing issue for TICK-NYSE Ticker (and all 'IND' sectype errors) *******************************************
        symbol = dataname
        sectype = self.p.sectype
        exch = self.p.exchange
        curr = self.p.currency
...
#ERIK ADDED FUNCTION BELOW********************************************************************************************************
     
    @staticmethod
    def throt_delay(self):
        #Needed to avoid Interactive Broker Pacing Restrictions for too many tickers requested
        throttime = 1 / self.reqlmt  # throttle delay in fraction of second
        self.reqcount += 1  # add one throt period
        self.reqcount -= int((time.time() - self.lastreqtime) / throttime)          # Decrement count by time passed:
        self.reqcount = max(self.reqcount, 0)  # keep it above zero
        self.lastreqtime = time.time()
        # sleep it off if the pacing is too fast...
        if self.reqcount >= 4 * self.reqlmt - 1:
            time.sleep(throttime)
...
#***********************************************************************************************************************************
    ...
    def reqdata(self):
        ...
        #**********************************ERIK ADDED*********************************************************
        if self.contract is None or (self._state == self._ST_LIVE and self.throt_delay(self)):
            return
		#*****************************************************************************************************
    ...
    def _load(self):
        ...
        while True:
            if self._state == self._ST_LIVE:
                ...
                #print("Requesting Historical Data EX from ibdata.py") #*************************ERIK ADDED*****************
                self.qhist = self.ib.reqHistoricalDataEx(
    ...
    def _load_rtbar(self, rtbar, hist=False):
        ...
        #print("LOADING STOCK DATA HERE IN LOAD_RTBAR") #****************************ERIK ADDED***********************
    ...
    def _load_rtvolume(self, rtvol):
        ...
        if dt < self.lines.datetime[-1] and not self.p.latethrough:
            return False  # cannot deliver earlier than already delivered
        #print("LOADING FOREX AND TICK-NYSE DATA HERE") #****************************ERIK ADDED**********************************
        self.lines.datetime[0] = dt
